...
Questa sezione identifica le minacce più comuni per la sicurezza del servizio SfBO e il modo in cui Microsoft mitiga ogni minaccia.
...
SfBO attenua questi attacchi eseguendo la protezione della rete DDOS di Azure e limitazione delle richieste client dagli stessi endpoint, subnet ed entità federate.
...
In questa sezione viene fornita una panoramica degli elementi fondamentali che costituiscono il framework di sicurezza per Microsoft SfBO. Questi elementi sono i seguenti:
...
L'autenticazione è il provisioning delle credenziali utente a un server o servizio attendibile. SfBO utilizza i seguenti protocolli di autenticazione, a seconda dello stato e della posizione dell'utente.
- **Autenticazione moderna** è l'implementazione Microsoft di OAUTH 2.0 per la comunicazione client-server. Abilita funzionalità di sicurezza come l'autenticazione basata su certificati, l'autenticazione a più fattori e l'accesso condizionale. Per usare MA, sia il tenant online sia i client devono essere abilitati per MA. I tenant SfBO creati dopo maggio 2017 hanno MA abilitata per impostazione predefinita. Per i tenant creati prima di allora, segui le istruzioni qui per attivarla. Tutti i seguenti client supportano MA: client Skype for Business 2015 o 2016, Skype for Business su Mac, client Lync 2013, telefoni IP 3PIP, iOS e Android. 
...
I certificati client offrono un metodo alternativo per l'autenticazione degli utenti da parte di SfBO. Anziché fornire un nome utente e una password, gli utenti hanno un certificato e la chiave privata corrispondente al certificato necessario per risolvere un'autenticazione crittografica. 
...
![Sequenza di chiamata in Partecipazione a una riunione.](media/sfbo-call-sequence-security.png) 
...
3. Il client invia una richiesta INFO per aggiungere l'utente alla conferenza A/V.
    ...
    [Nota]  Tutto il traffico SIP precedente scorreva attraverso il servizio Access Edge.
...
4. Tra il client e A/V Conferencing Server, una connessione multimediale viene negoziata e configurata tramite SRTP.
...
- **Organizzatore** &nbsp;&nbsp; L'utente che crea una riunione, sia estemporanea che in base alla pianificazione. Un organizzatore deve essere un utente aziendale autenticato e avere il controllo su tutti gli aspetti relativi all'utente finale di una riunione.
- **Presentatore** &nbsp;&nbsp; Un utente autorizzato a presentare informazioni a una riunione, usando qualsiasi elemento multimediale supportato. Un organizzatore della riunione è per definizione anche un relatore e determina chi altro possa essere un relatore. Un organizzatore può prendere questa decisione quando è in programma una riunione o mentre la riunione è in corso.
- **Partecipante** &nbsp;&nbsp; Un utente che è stato invitato a partecipare a una riunione ma che non è autorizzato ad agire come relatore.
...
1.  **Utenti che appartengono al tenant** &nbsp;&nbsp; Questi utenti hanno una credenziale in Azure Active Directory per il tenant.<br/>
    a. *All'interno di corpnet* - Questi utenti si aggiungono dall'interno della rete aziendale.<br/>b. *Utenti remoti*: questi utenti partecipano dall'esterno della rete aziendale. Possono comprendere i dipendenti che lavorano da casa o in viaggio e altri, come dipendenti di fornitori affidabili, a cui sono state concesse credenziali aziendali per le rispettive condizioni d'uso. Gli utenti remoti possono creare e partecipare a conferenze e agire da relatori.
2.  **Utenti che non appartengono al tenant**&nbsp;&nbsp;  Questi utenti non dispongono di credenziali in Azure Active Directory per il tenant.<br/>a. *Utenti federati* : gli utenti federati dispongono di credenziali valide con partner federati e pertanto vengono trattati come autenticati da SfBO. Gli utenti federati possono partecipare alle conferenze ed essere promossi a relatori dopo che hanno aderito alla riunione, ma non possono creare conferenze in aziende con cui sono federati.<br/>b. *Utenti anonimi*: gli utenti anonimi non hanno un'identità di Active Directory e non sono federati con il tenant. 
...
- **Tutti, incluse le persone esterne all'azienda**&nbsp;&nbsp; Tutti gli utenti che partecipano alla riunione possono partecipare alla riunione senza restrizioni.
