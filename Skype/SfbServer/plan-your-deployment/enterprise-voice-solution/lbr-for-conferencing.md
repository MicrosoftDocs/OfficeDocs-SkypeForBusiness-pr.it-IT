...
Location-Based routing consente di limitare il routing delle chiamate tra gli endpoint VoIP e gli endpoint PSTN in base alla posizione delle parti nella chiamata. Location-Based routing per conferenze consente di applicare Location-Based regole di routing alle riunioni (ad esempio, conferenze) per impedire il bypass a pedaggio PSTN. L'applicazione monitora una conferenza attiva e applica Location-Based routing in base alla posizione degli utenti che partecipano. L'Location-Based routing per conferenze consente inoltre l'applicazione delle restrizioni Location-Based routing ai trasferimenti di consulenza che coinvolgono endpoint PSTN.

L'Location-Based Routing Conferencing fornisce alle conferenze Skype for Business un meccanismo per la prevenzione del bypass a pedaggio PSTN. L'applicazione monitora le conferenze attive e applica Location-Based routing in base alla posizione degli utenti Skype for Business partecipanti.

LLocation-Based app di Routing Conferencing determina se il Location-Based deve essere applicato a una riunione Skype for Business se vengono soddisfatti i criteri seguenti:
...
- Se organizzatori e partecipanti aderiscono alla riunione dallo stesso sito di rete e vi sono partecipanti che aderiscono alla stessa riunione dalla rete PSTN, non è consentito partecipare alla riunione a un endpoint Skype for Business da un sito di rete diverso.
...
- Un endpoint PSTN che partecipa a una conferenza con imposizioni di routing Location-Based non sarà limitato a partecipare alla conferenza indipendentemente dallo stato se l'endpoint si unisce tramite un trunk non abilitato per il routing Location-Based.

- Un sistema PBX connesso a un Mediation Server tramite un trunk SIP che non evade le chiamate alla rete PSTN avrà le stesse imposizioni degli utenti di Skype for Business che si trovano nello stesso sito di rete in cui è definito il trunk SIP. Ad esempio, un endpoint PSTN potrà partecipare a una conferenza con un utente PBX e un utente di Skype for Business se si trova nello stesso sito di rete. In caso contrario, l'endpoint PSTN non potrà partecipare alla conferenza se l'utente PBX si trova in un sito di rete diverso da quello dell'utente di Skype for Business.
...
|:-----|:-----|:-----|:-----|
|Skype for Business Mobile  <br/> |PSTN  <br/> |Skype for Business Mobile è in una chiamata PSTN. Skype for Business Mobile quindi riassalta la chiamata a un'autorità di Operatore automatico (CAA).  <br/> |La chiamata è bloccata, con un messaggio di errore appropriato.  <br/> |
...
L'applicazione Location-Based Routing per conferenze richiede che Skype for Business Server o Lync Server 2013 cumulative Update 2 sia distribuito in tutti i pool Front-End e i edizione Standard Server della topologia. Se queste versioni del server non sono installate in alcuni server della topologia, Location-Based le restrizioni di routing non possono essere applicate completamente alle riunioni e ai trasferimenti di chiamata consultiva.
...
- Per applicare Location-Based routing delle riunioni, Skype for Business i partecipanti devono essere abilitati per Location-Based routing.
...
L'Location-Based routing per conferenze è disabilitata per impostazione predefinita. Prima di abilitare questa applicazione, è necessario determinare la priorità giusta da assegnare all'applicazione. Per determinare questa priorità, eseguire il cmdlet seguente in Skype for Business Server Management Shell:

```powershell
Get-CsServerApplication -Identity Service:Registrar:<Pool FQDN>
```

In questo cmdlet è \<Pool FQDN\> il pool in cui deve essere Location-Based'applicazione routing per conferenze.
...
Ad esempio, se l'applicazione "UdcAgent" ha un valore di priorità "2", l'applicazione "DefaultRouting" ha un valore di priorità "8", l'applicazione "ExumRouting" ha un valore di priorità "9" e l'applicazione "OutboundRouting" ha un valore di priorità "10", è necessario assegnare all'applicazione Location-Based Routing per conferenze un valore di priorità pari a "3". In questo modo, la priorità delle applicazioni viene posizionata nell'ordine seguente: altre applicazioni (priorità: da 0 a 1), "UdcAgent" (priorità: 2), Location-Based Routing Conferencing application (Priority: 3), altre applicazioni (priorità: 4 a 8), "DefaultRouting" (Priority: 9), "ExumRouting" (Priority: 10) e "OutboundRouting" (Priority: 11).
