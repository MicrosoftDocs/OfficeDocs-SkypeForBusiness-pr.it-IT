...
description: Pianificazione del routing in base alla posizione per le conferenze in Skype for Business Server VoIP aziendale, inclusi i trasferimenti di chiamata consultivi.
ms.openlocfilehash: d9fee0304310f551d750be85a9b6e2b0d9a47ad3
ms.sourcegitcommit: 67324fe43f50c8414bb65c52f5b561ac30b52748
ms.translationtype: MT
ms.contentlocale: it-IT
ms.lasthandoff: 11/08/2021
ms.locfileid: "60855403"
---
...
Pianificazione del routing in base alla posizione per le conferenze in Skype for Business Server VoIP aziendale, inclusi i trasferimenti di chiamata consultivi.

Location-Based routing consente di limitare il routing delle chiamate tra gli endpoint VoIP e gli endpoint PSTN in base alla posizione delle parti nella chiamata. Location-Based routing per conferenze consente di applicare regole Location-Based routing alle riunioni (ad esempio, conferenze) per impedire il bypass a pedaggio PSTN. L'applicazione monitora una conferenza attiva e applica Location-Based instradamento in base alla posizione degli utenti che partecipano. L'Location-Based routing per conferenze consente inoltre l'applicazione delle restrizioni di routing Location-Based ai trasferimenti di consulenza che coinvolgono endpoint PSTN.

L'Location-Based Routing Conferencing fornisce alle conferenze Skype for Business un meccanismo per la prevenzione del bypass a pedaggio PSTN. L'applicazione monitora le conferenze attive e applica Location-Based routing in base alla posizione Skype for Business utenti che partecipano.

L'Location-Based Routing Conferencing determina se il routing Location-Based deve essere applicato a una riunione Skype for Business se vengono soddisfatti i criteri seguenti:
...
L'Location-Based routing per conferenze impedisce la partecipazione di utenti Skype for Business endpoint PSTN da siti di rete diversi alla stessa conferenza. Se l'organizzatore di una riunione è abilitato Location-Based routing, l'applicazione di conferenza applica le restrizioni seguenti:
...
- Un endpoint PSTN che partecipa a una conferenza con Location-Based Le imposizioni del routing non saranno limitate a partecipare alla conferenza indipendentemente dal relativo stato se l'endpoint si unisce tramite un trunk non abilitato per il routing Location-Based.
...
|User|Other Party|Azione|Risultato|
|:-----|:-----|:-----|:-----|
...
L'applicazione Location-Based routing per conferenze richiede che Skype for Business Server o Lync Server 2013 cumulative Update 2 sia distribuito in tutti i pool Front-End e i edizione Standard Server della topologia. Se queste versioni del server non sono installate in alcuni server della topologia, Location-Based le restrizioni di routing non possono essere applicate completamente alle riunioni e ai trasferimenti di chiamate consultiva.
...
In questo cmdlet è il pool in cui deve essere abilitata \<Pool FQDN\> Location-Based'applicazione routing per conferenze.
...
Ad esempio, se l'applicazione "UdcAgent" ha un valore di priorità "2", l'applicazione "DefaultRouting" ha un valore di priorità "8", l'applicazione "ExumRouting" ha un valore di priorità "9" e l'applicazione "OutboundRouting" ha un valore di priorità "10", è necessario assegnare all'applicazione Location-Based Routing per conferenze un valore di priorità pari a "3". In questo modo, la priorità delle applicazioni viene posizionata nell'ordine seguente: altre applicazioni (priorità: da 0 a 1), "UdcAgent" (priorità: 2), Location-Based Applicazione di routing per conferenze (priorità: 3), altre applicazioni (priorità: da 4 a 8), "DefaultRouting" (Priorità: 9), "ExumRouting" (Priorità: 10) e "OutboundRouting" (Priorità: 11).

Dopo aver trovato il valore di priorità corretto per l'applicazione Location-Based Routing per conferenze, digitare il cmdlet seguente per ogni pool di Front-End o edizione Standard Server in cui sono abilitati gli utenti per il routing Location-Based:
