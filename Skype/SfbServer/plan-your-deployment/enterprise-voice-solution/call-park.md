...
L'applicazione Parcheggio di VoIP aziendale consente agli utenti di eseguire le operazioni seguenti:
...
L'applicazione Parcheggio di chiamata viene installata automaticamente quando si distribuisce VoIP aziendale. È possibile abilitare il parcheggio di chiamata configurando i criteri vocali.
  
### <a name="software-requirements"></a>Requisiti software

In tutti i Front End Server e nei server Standard Edition in cui è distribuito il parcheggio di chiamata deve essere installato Runtime formato Windows Media per i server che eseguono Windows Server 2008 R2 o Microsoft Media Foundation per i server che eseguono Windows Server 2012 o Windows Server 2012 R2. Per Windows Server 2008 R2, Runtime formato Windows Media viene installato come parte di Esperienza desktop di Windows. Runtime formato Windows Media o Microsoft Media Foundation è necessario per i file Windows Media Audio (wma) riprodotti da Parcheggio di chiamata per la musica di attesa.
...
I client e i tipi di chiamate seguenti sono supportati per il parcheggio di chiamata
...
> [!NOTE]
> I telefoni cellulari non possono utilizzare parcheggio di chiamata per parcheggiare le chiamate. 
...
- Telefoni non IP connessi all'infrastruttura di Skype for Business Server, inclusi telefoni delle aree comuni e telefoni PBX (Private Branch Exchange)
