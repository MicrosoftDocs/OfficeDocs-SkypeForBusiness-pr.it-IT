...
description: Decisioni necessarie per la pianificazione di un database delle informazioni sulla posizione o di un database esterno simile per una distribuzione E9-1-1 con provider di trunking SIP, in Skype for Business Server VoIP aziendale.
...
Decisioni necessarie per la pianificazione di un database delle informazioni sulla posizione o di un database esterno simile per una distribuzione E9-1-1 con provider di trunking SIP, in Skype for Business Server VoIP aziendale.
...
Il database del servizio informazioni percorso viene popolato con un percorso di risposta per gli interventi di emergenza (ERL, Emergency Response Location), costituito da un indirizzo civico e dall'indirizzo specifico all'interno di un edificio. Il campo Posizione del servizio **informazioni** percorso, che rappresenta la posizione specifica all'interno di un edificio, ha una lunghezza massima di 20 caratteri (spazi inclusi). Tentare di includere, in tale lunghezza, gli elementi seguenti:
...
Nelle sezioni seguenti vengono illustrate le considerazioni da tenere in considerazione quando si popola e si mantiene il database delle località.

## <a name="populating-the-location-database"></a>Popolamento del database delle posizioni

Le domande seguenti possono essere utili per determinare come popolare il database delle località.
