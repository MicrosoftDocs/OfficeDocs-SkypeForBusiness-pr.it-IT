...
Informazioni sul controllo di ammissione di chiamata, che può impedire che le chiamate avvengano se avessero una qualità media scadente, in Skype for Business Server VoIP aziendale.

Per le applicazioni basate su IP, come la telefonia, il video e la condivisione di applicazioni, la larghezza di banda disponibile per le reti aziendali non è in genere considerata un fattore di limitazione all'interno degli ambienti LAN. Nei collegamenti WAN tra i siti tuttavia la larghezza di banda della rete può essere limitata.

Quando il traffico di rete Annulla la sottoscrizione di un collegamento WAN, vengono utilizzati meccanismi di Accodamento, di buffer e di eliminazione dei pacchetti per risolvere la congestione. Il traffico in eccesso in genere viene ritardato finché non si risolve la congestione di rete o, se necessario, finché non viene eliminato il traffico. Per il traffico di dati convenzionali, in questi casi è possibile ripristinare il client di ricezione. Tuttavia, per il traffico in tempo reale, come le comunicazioni unificate, la congestione della rete non può essere risolta in questo modo, perché il traffico di comunicazioni unificate è sensibile alla latenza e alla perdita di pacchetti. La congestione nella WAN può determinare una qualità percepita dagli utenti (QoE) scadente. Per il traffico in tempo reale in condizioni congestionate, è preferibile negare le chiamate piuttosto che fornire connessioni con qualità scadente.
...
La progettazione di CAC in Skype for Business Server offre quattro attributi principali:
...
Gli amministratori definiscono i criteri di CAC, applicati dal servizio criteri di larghezza di banda installato con tutti i pool Front end. Le impostazioni di CAC vengono propagate automaticamente a tutti i Front End Server Skype for business in rete.
...
Anche se la limitazione della larghezza di banda per l'audio e il video viene applicata dal servizio criteri di larghezza di banda in Skype for Business Server, questa restrizione non viene applicata al router di rete (Layer 2 e 3). Il controllo di ammissione di chiamata non può impedire a un'applicazione dati, ad esempio, di consumare l'intera larghezza di banda di rete su un collegamento WAN, inclusa la larghezza di banda riservata per l'audio e il video dal criterio CAC. Per proteggere la larghezza di banda necessaria per la rete, è possibile distribuire un protocollo QoS (Quality of Service), ad esempio servizi differenziati (DiffServ). Pertanto, una procedura consigliata consiste nel coordinare i criteri di larghezza di banda di CAC definiti con eventuali impostazioni QoS che è possibile distribuire.
...
I componenti di questa topologia di esempio sono spiegati nelle sezioni seguenti. Per informazioni dettagliate sulla pianificazione della topologia, compresi i limiti relativi alla larghezza di banda, vedere [example: Gathering requirements for Call Admission Control in Skype for Business Server](example-gathering-requirements.md).
...
Un sito di rete rappresenta una località in cui l'organizzazione dispone di spazi fisici, ad esempio uffici, un gruppo di edifici o un campus. Uno spazio fisico con connettività LAN e WAN ad altri siti è considerata un sito di rete. Iniziare a inventariare tutti gli uffici dell'organizzazione. Nella topologia dell'esempio, l'area di rete Nord America è costituita dai siti di rete seguenti: New York, Chicago, Detroit, Portland, Reno e Albuquerque.
...
- Le chiamate audio peer-to-peer tra gli endpoint di Skype for business utilizzeranno RTAudio (8kHz) o RTAudio (16kHz) quando si calcola la larghezza di banda e la priorità dei codec.
...
- Le chiamate alla rete PSTN (Public Switched Telephone Network) a o da endpoint Skype for business utilizzeranno G. 711 o RTAudio (8kHz).
...
Il codec Siren viene utilizzato in Skype for Business Server nei casi seguenti:

- Se i criteri di larghezza di banda sono impostati su un valore troppo basso per l'uso di G.722

- Se un client Communications Server 2007 o Communications Server 2007 R2 si connette a un servizio di conferenza di Skype for Business Server (poiché tali client non supportano il codec G. 722).
