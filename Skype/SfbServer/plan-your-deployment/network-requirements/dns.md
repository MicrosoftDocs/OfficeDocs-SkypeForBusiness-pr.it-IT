...
**Riepilogo:** Leggere le considerazioni relative al DNS in questo argomento prima di implementare Skype for Business Server.
...
|:-----|:-----|:-----|:-----|:-----|
|A/AAAA   |Front End pool FQDN  <br/> *FE-pool.<span></span> contoso.com <span></span>*   |Indirizzi IP del server del pool Front End  <br/>  DNS LB a *192.168.21.122 192.168.21.123 192.168.21.124*   |Bilanciamento del carico DNS dei pool Front End. Mappe il nome del pool Front End in un set di indirizzi IP.  <br/> Vedere [Deploying DNS Load Balancing on Front End Pools and Director Pools](load-balancing.md#BK_FE_Dir)  |Y   |
|A/AAAA   | FQDN di ogni Front End Server o edizione Standard server in un pool o in un server autonomo <br/>  *FE01.<span></span> contoso.<span></span> com FE02.<span></span> contoso.com <span></span> FE03.<span></span> contoso.com <span></span>*   |IP corrispondente di ogni server  <br/> *192.168.21.122 192.168.21.123 192.168.21.124*   |Mappe il nome del server al relativo indirizzo IP.   |Y   |
|A/AAAA   |Enterprise dei servizi Web interni del pool sostituisce il nome di dominio completo  <br/> *Web-int.<span></span> contoso.com <span></span>*   |VIP HLB per i servizi Web interni del Front End Server  <br/> *192.168.21.120*   |Necessario per abilitare il traffico Web da client a server, ad esempio il download del Skype for Business Web App. Obbligatorio anche per i client mobili.   |Y   |
|A/AAAA   |Enterprise pool di servizi Web esterni sostituisce il nome di dominio completo  <br/> *Web-ext.<span></span> contoso.com <span></span>*   |VIP di bilanciamento del carico di rete per i servizi Web esterni del Front End Server  <br/>*68.123.56.90*   |Necessario per abilitare il traffico Web da client a server, ad esempio il download del Skype for Business Web App. Obbligatorio se i client mobili risolvono il DNS internamente. Può essere risolto in IP proxy inverso DMZ o IP Internet.   ||
|A/AAAA   | Fqdn server back-end SQL server <br/> *SQL1.<span></span> contoso.com <span></span>*   |indirizzo IP del server  <br/> *192.168.11.90*   |Mappe il nome del server per un server SQL back-end che lavora con il pool Front End al relativo indirizzo IP   ||
|A/AAAA   |Fqdn del server mirror back-SQL server back-end  <br/> *SQL2.<span></span> contoso.com <span></span>*   |indirizzo IP del server  <br/> *192.168.11.91*   |Mappe il nome del server per un server mirror SQL back-end che lavora con il pool Front End al relativo indirizzo IP   ||
|A/AAAA   |Director pool FQDN  <br/>**Nota:** Non applicabile quando si utilizza un server Director autonomo <br/> *DirPool.<span></span> contoso.com <span></span>*   |Indirizzi IP del pool di server Director  <br/> DNS LB a *192.168.21.132, 192.168.21.133, 192.168.21.134*   |Bilanciamento del carico DNS dei server del pool di server Director. Mappe il nome del pool di server Director in un indirizzo IP, vedere [Deploying DNS Load Balancing on Front End Pools and Director Pools](load-balancing.md#BK_FE_Dir) <br/> Un Director può autenticare un utente ed è facoltativo.   ||
|A/AAAA   |Director FQDN   |Indirizzo IP server di ogni server Director   |Mappe il nome del pool per il Director in un indirizzo IP, vedere [Deploying DNS Load Balancing on Front End Pools and Director Pools](load-balancing.md#BK_FE_Dir)  ||
...
|A/AAAA   |lyncdiscoverinternal.*\<sipdomain\>* <br/> lyncdiscoverinternal. *<span></span>contoso.com <span></span>*   |VIP o IP director del pool Front End HLB  <br/>  192.168.21.121  |Servizio di individuazione automatica interno1, necessario per il supporto per dispositivi mobili. Se il DNS interno viene usato per la risoluzione per i dispositivi mobili, deve puntare all'IP esterno o al VIP DMZ.  <br/> Per i servizi Web è necessario il bilanciamento del carico di rete nel pool Front End perché HTTPS non è in grado di sfruttare DNS. Per il pool Front End o il pool di server Director, questa operazione deve essere risolta in un VIP di bilanciamento del carico di rete o in un indirizzo IP normale per un server Standard Edition o un server Director autonomo.   |Y   |
|CNAME   |lyncdiscoverinternal.*\<sipdomain\>* <br/> lyncdiscoverinternal. *<span></span>contoso.com <span></span>*   |FQDN pool FE HLB o FQDN director  <br/> Web-int.<span></span> contoso.com<span></span>   |Servizio di individuazione automatica interno1 <br/> Se lo si desidera, è possibile implementare questo metodo come CNAME anziché come record A.   ||
|A/AAAA   |sip.*\<sipdomain\>* <br/> sip. *<span></span>contoso.com <span></span>*  |Indirizzi IP del server del pool Front End (o a ogni indirizzo IP del server Director)  <br/>  DNS LB a *192.168.21.122 192.168.21.123 192.168.21.124*   |Obbligatorio per la configurazione automatica, vedere [Procedura dettagliata Skype for Business client che individuano i servizi](../../plan-your-deployment/edge-server-deployments/advanced-edge-server-dns.md#WalkthroughOfSkype) <br/> Record o record che puntano ai server del pool Front End o ai server Director nella rete interna o al servizio Access Edge quando il client è esterno   |&#x2777;  |
|A/AAAA   |ucupdates-r2.*\<sipdomain\>* <br/> ucupdates-r2. *<span></span>contoso.com <span></span>*  |VIP del pool FE HLB o VIP di bilanciamento del carico di rete del pool di server Director o IP edizione Standard/server Director  <br/>  192.168.21.121  |La distribuzione di questo record è facoltativa &#x2778;  ||
|SRV   |\_sipinternaltls.\_ tcp.*\<sipdomain\>* <br/>Porta 5061 <br/>\_sipinternaltls.\_ tcp. *<span></span>contoso.com <span></span>* <br/>Porta 5061  |Front End pool FQDN  <br/>*FE-Pool.<span></span> contoso.com <span></span>*  |Abilita l'accesso automatico dell'utente interno 1 al front-end server/pool o edizione Standard server/pool che autentica e reindirizza le richieste client per l'accesso.  |&#x2777; |
|A/AAAA |sipinternal.*\<sipdomain\>* <br/>sipinternal.<span></span> *contoso.com <span></span>*  |Front End pool FQDN  <br/>_FE-Pool.<span></span> contoso.com <span></span>_  |Accesso utente interno &#x2776;  |&#x2777;  |
|SRV   | \_ntp.\_ udp.*\<sipdomain\>* <br/> \_ntp.\_ udp.<span></span> *contoso.com <span></span>*  |TimeServer FQDN  <br/> north-america.pool.ntp.org   |Origine NTP necessaria per i dispositivi Lync Telefono Edition   |Questa operazione è necessaria per supportare i portatili desktop.   |
|SRV   |\_sipfederationtls.\_ tcp.*\<sipdomain\>* <br/>\_sipfederationtls.\_ tcp.<span></span> *contoso.com <span></span>*  | FQDN del servizio Access Edge <br/> EdgePool-int.<span></span> *contoso.com <span></span>*  |Creare un record SRV per ogni dominio SIP che dispone di client IOS o Windows telefoni cellulari.   |Per il supporto di client per dispositivi mobili   |
|A/AAAA   |URL amministratore  <br/>*Web-int.<span></span> contoso.com <span></span>*  |HLB FE Pool VIP  <br/> 192.168.21.121   |Skype for Business Server pannello di controllo, vedere [URL semplici](dns.md#BK_Simple)  ||
|A/AAAA   |URL di meet  <br/>*Web-int.<span></span> contoso.com <span></span>*  |HLB FE Pool VIP  <br/> 192.168.21.121   |Riunioni online, vedere [URL semplici](dns.md#BK_Simple)  ||
|A/AAAA   |URL di accesso esterno  <br/>*Web-int.<span></span> contoso.com <span></span>*  |HLB FE Pool VIP  <br/> 192.168.21.121   |Conferenze telefoniche con accesso esterno, vedere [URL semplici](dns.md#BK_Simple)  ||
|A/AAAA   |FQDN dei servizi Web interni  <br/>*Web-int.<span></span> contoso.com <span></span>*  |HLB FE Pool VIP  <br/> 192.168.21.121   |Skype for Business Web utilizzato da Skype for Business Web App   ||
|A/AAAA   |Office FQDN del pool di server Web Apps  <br/> OWA.<span></span> contoso.com<span></span>   | Office VIP del pool di server Web Apps <br/> 192.168.1.5   |Definisce l'FQDN Office del pool di server Web Apps   ||
|A/AAAA   | FQDN Web interno <br/> Web-int.<span></span> contoso.com<span></span>   | Indirizzo VIP del pool Front End <br/> 192.168.21.121   |Definisce l'FQDN Web interno utilizzato da Skype for Business Web App  <br/> Se si utilizza il bilanciamento del carico DNS in questo pool, il pool Front End e la web farm interna non possono avere lo stesso FQDN.   ||

&#x2776; Utilizzato da un client per individuare il Front End Server o il pool Front End ed essere autenticato e connesso come utente. Per ulteriori dettagli, vedere [Walkthrough of Skype for Business clients locating services](../../plan-your-deployment/edge-server-deployments/advanced-edge-server-dns.md#WalkthroughOfSkype).
...
|Tipo di record|Valore|Viene risolto in|Finalità|Obbligatorio|
|:--- |:--- |:--- |:--- |:--- |
|A/AAAA   |FQDN del pool di server perimetrali interno  <br/>*EdgePool-int.<span></span> contoso.com <span></span>*  |Indirizzi IP del pool di server perimetrali con connessione interna  <br/> 172.25.33.10, 172.25.33.11   |Indirizzi IP dell'interfaccia interna del pool di server perimetrali consolidato   |Y   |
|A/AAAA   |FQDN server perimetrale  <br/>*Cons-1.<span></span> contoso.com <span></span>*  |IP del server con connessione interna per un server nel pool di server perimetrali  <br/> 172.25.33.10   |Creare un record per ogni server del pool con l'FQDN del server che punta all'IP del nodo del server interno nel pool, vedere [DNS Load Balancing on Edge Server Pools](load-balancing.md#BK_Edge).   |Y   |
|A/AAAA   |FQDN del pool di servizi Access Edge  <br/>*Access1.<span></span> contoso.com <span></span>*  |Indirizzi IP esterni del pool di servizi Access Edge  <br/> 131.107.16.10, 131.107.16.11   |Questo servizio fornisce un singolo punto di connessione attendibile per il traffico SIP (Session Initiation Protocol) sia in ingresso che in uscita.   |Y   |
|A/AAAA   |FQDN del pool di servizi Web Conferencing Edge  <br/>*Webcon1.<span></span> contoso.com <span></span>*  |Indirizzi IP esterni del servizio Web Conferencing Edge  <br/> 131.107.16.90, 131.107.16.91   |Il servizio Web Conferencing Edge consente agli utenti esterni di partecipare alle riunioni ospitate nell'ambiente Skype for Business Server interno.   |Y   |
|A/AAAA   |*av.\<sip-domain\>* FQDN pool <br/>*AV1.<span></span> contoso.com <span></span>*  |Indirizzi IP esterni A/V Edge  <br/> 131.107.16.170, 131.107.16.171   |Il servizio A/V Edge rende disponibili agli utenti esterni audio, video, condivisione applicazioni e trasferimento di file.   |Y   |
|CNAME   |sip.*\<sipdomain\>* <br/> sip. *<span></span>contoso.com <span></span>*  |FQDN del pool di server perimetrali di accesso esterno  <br/>*Access1.<span></span> contoso.com <span></span>*  |Individua il pool di server perimetrali . Vedere [Procedura dettagliata di individuazione Skype for Business client che individuano i servizi](../../plan-your-deployment/edge-server-deployments/advanced-edge-server-dns.md#WalkthroughOfSkype)  |Y   |
|SRV   |\_sip.\_ tls.*\<sipdomain\>* <br/>\_sip.\_ tls.<span></span> *contoso.com <span></span>*  |FQDN access edge esterno  <br/>_Access1.<span></span> contoso.com <span></span>_  |Utilizzato per l'accesso degli utenti esterni. Vedere [Procedura dettagliata di individuazione Skype for Business client che individuano i servizi](../../plan-your-deployment/edge-server-deployments/advanced-edge-server-dns.md#WalkthroughOfSkype)  |Y   |
|SRV   |\_sipfederationtls.\_ tcp.*\<sipdomain\>* <br/>\_sipfederationtls.\_ tcp.<span></span> *contoso.com <span></span>*  |FQDN access edge esterno  <br/>*Access1.<span></span> contoso.com <span></span>*  |Usato per la federazione e la connettività di messaggistica istantanea pubblica   |&#x2776;  |
|SRV   |\_xmpp-server.\_ tcp. *<sipdomain\>* <br/>\_xmpp-server.\_ tcp. *<span></span>contoso.com <span></span>*  |FQDN access edge esterno  <br/>*Access1.<span></span> contoso.com <span></span>*  |Il servizio proxy XMPP accetta e invia messaggi XMPP (Extensible Messaging and Presence Protocol) a e da partner federati XMPP configurati.   |Y, per distribuire la federazione, altrimenti facoltativo  <br/> Non disponibile in Skype for Business Server 2019.|
|SRV   |\_sipfederationtls.\_ tcp.*\<sipdomain\>* <br/>\_sipfederationtls.\_ tcp. *<span></span>contoso.com <span></span>*  |FQDN access edge esterno  <br/>*Access1.<span></span> contoso.com <span></span>*  |Per supportare il servizio di notifica Push e il servizio notifica Push Apple, devi creare un record SRV per ogni dominio SIP. &#x2778;  ||
|A/AAAA   |FQDN dei servizi Web del pool Front End esterno  <br/>*Web-ext.<span></span> contoso.com <span></span>*  |Indirizzo IP pubblico proxy inverso, proxy all'indirizzo VIP dei servizi Web esterni per il pool Front End &#x2776; <br/> 131.107.155.1 proxy a 192.168.21.120   |Interfaccia esterna del pool Front End utilizzata da Skype for Business Web App   |Y   |
|A/AAAA/CNAME   |lyncdiscover.*\<sipdomain\>* <br/> lyncdiscover. *<span></span>contoso.com <span></span>*  |Indirizzo IP pubblico proxy inverso, viene risolto nell'indirizzo VIP servizi Web esterni per il pool di server Director, se disponibile, o per il pool Front End se non si dispone di un indirizzo &#x2777; <br/> 131.107.155.1 proxy a 192.168.21.120   | Record esterno per l'individuazione automatica del client, usato anche da Mobility, Skype for Business Web App e dall'app Web dell'utilità di pianificazione, risolto dal server proxy inverso <br/> Per supportare il servizio di notifica Push e il servizio notifica Push Apple, creare un record SRV per ogni dominio SIP con client Microsoft Lync Mobile. 3   |Y   |
|A/AAAA   |meet.*\<sipdomain\>* <br/> meet. *<span></span>contoso.com <span></span>*  |Indirizzo IP pubblico proxy inverso, risolto nell'interfaccia Web esterna per il pool Front End  <br/> 131.107.155.1 proxy a 192.168.21.120   |Proxy per Skype for Business Web  <br/> Vedere [URL semplici](dns.md#BK_Simple)  |Y   |
|A/AAAA   |accesso remoto.*\<sipdomain\>* <br/> accesso remoto. *<span></span>contoso.com <span></span>*  |Indirizzo IP pubblico proxy inverso, proxy all'interfaccia Web esterna per il pool Front End  <br/> 131.107.155.1 proxy a 192.168.21.120   |Proxy per Skype for Business Web  <br/> Vedere [URL semplici](dns.md#BK_Simple)  |Y   |
|A/AAAA   |Office FQDN del pool di server Web Apps  <br/> OWA.<span></span> contoso.com<span></span>   | Indirizzo IP pubblico proxy inverso, proxy all'interfaccia Web esterna per il Office Web Apps Server <br/> 131.107.155.1 proxy a 192.168.1.5   | Office VIP del pool di server Web Apps <br/> 192.168.1.5   |Definisce l'FQDN Office del pool di server Web Apps   |
...
> [!IMPORTANT]
> Skype for Business Server supporta l'utilizzo dell'indirizzamento IPv6. Per [ulteriori dettagli, vedere Plan for IPv6 in Skype for Business](ipv6.md).
...
È inoltre consigliabile fare riferimento a "URL Office 365 e intervalli di indirizzi IP" [https://aka.ms/o365ips](/microsoft-365/enterprise/urls-and-ip-address-ranges) all'indirizzo per verificare che gli utenti avranno accesso alle risorse online necessarie.
...
|:-----|:-----|
|One Server:  <br/> Persistent Chat, Director, Mediation Server, Front end Server   |Un record A interno che risolve il nome di dominio completo (FQDN) del server nel relativo indirizzo IP.  <br/> ServerRole.<span></span> contoso.com<span></span> 10.10.10.0   |
|Pool:  <br/> Persistent Chat, Director, Edge Server, Mediation Server, Front end   |Un record A interno che risolve il nome di dominio completo (FQDN) di ogni nodo del server nel pool nel relativo indirizzo IP.  <br/>**Esempio** <br/> ServerRole01.<span></span> contoso.com<span></span> 10.10.10.1  <br/> ServerRole02.<span></span> contoso.com<span></span> 10.10.10.2  <br/> Più record A interni che risolvono il nome di dominio completo (FQDN) del pool negli indirizzi IP dei nodi del server nel pool.  <br/>**Esempio** <br/> ServerPool.<span></span> contoso.com<span></span> 10.10.10.1  <br/> ServerPool.<span></span> contoso.com<span></span> 10.10.10.2   |

### <a name="edge-server-specific-dns-topics"></a>Argomenti DNS specifici del server perimetrale

 Per pianificare la distribuzione dei server perimetrali, vedere Plan [for Edge Server deployments in Skype for Business Server 2015](../../plan-your-deployment/edge-server-deployments/edge-server-deployments.md) e [Advanced Edge Server DNS planning for Skype for Business Server 2015 con](../../plan-your-deployment/edge-server-deployments/advanced-edge-server-dns.md) le sezioni seguenti
