...
Skype for Business Server fornisce i modi seguenti per configurare Office web apps server/Office Online Server. A seconda delle esigenze, è possibile:
...
- **Usa un server web Office/Office Online Server distribuito esternamente.** In questa topologia, Skype for Business Server viene distribuito in locale e usa un server/Office Online Server Web Apps Office distribuito all'esterno dell'area di rete Skype for Business Server. Ciò può verificarsi quando Office Web Apps Server/Office Online Server viene condiviso tra più applicazioni dell'azienda e viene distribuito in una rete che richiede a Skype for Business Server di utilizzare l'interfaccia esterna di Office Web Apps Server/Office Online Server e viceversa.
    ...
    Se il Office Online Server/server web apps Office viene distribuito all'esterno del firewall interno, selezionare l'opzione **Office Il server Web Apps viene** distribuito in una rete esterna, ovvero perimetrale/Internet, in Generatore di topologie.
