...
Il monitoraggio offre inoltre un meccanismo che consente agli endpoint SIP (ad esempio Skype for Business) di fornire informazioni sulla risoluzione dei problemi a cui l'amministratore non avrebbe altrimenti accesso:
...
Tenere presente che è SQL Server essere installato e configurato prima di distribuire e configurare il monitoraggio. Tuttavia, è solo necessario distribuire SQL Server se stesso. i database di monitoraggio verranno creati automaticamente quando si pubblica la Skype for Business Server topologia.
