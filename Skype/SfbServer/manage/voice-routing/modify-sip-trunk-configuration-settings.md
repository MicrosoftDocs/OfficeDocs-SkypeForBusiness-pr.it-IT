...
Quando si installa Skype for Business Server, viene creata una raccolta globale di impostazioni di configurazione del trunk SIP. Gli amministratori inoltre possono creare raccolte di impostazioni personalizzate nell'ambito del sito o del servizio (solo per il servizio gateway PSTN). Qualsiasi raccolta può essere modificata in un secondo momento utilizzando il pannello di controllo di Skype for Business Server o Windows PowerShell.
