---
title: Pagina Aggiungere un server perimetrale con FQDN singolo
ms.reviewer: ''
ms.author: v-lanac
author: lanachin
manager: serdars
ms.date: 4/14/2015
audience: ITPro
ms.topic: article
f1.keywords:
- NOCSH
ms.custom:
- ms.lync.tb.AddEdgeServerIsSingleFqdnPage
ms.prod: skype-for-business-itpro
localization_priority: Normal
ms.assetid: 44d98453-cb41-49cd-99cb-52b6ebd2d559
description: Pagina Informazioni sull'FQDN singolo del server perimetrale
ms.openlocfilehash: a9a4fcd3f596fac06922cea047ead9cbf29c47f6
ms.sourcegitcommit: e64c50818cac37f3d6f0f96d0d4ff0f4bba24aef
ms.translationtype: MT
ms.contentlocale: it-IT
ms.lasthandoff: 02/06/2020
ms.locfileid: "41820958"
---
# <a name="add-edge-server-with-single-fqdn-page"></a>Pagina Aggiungere un server perimetrale con FQDN singolo
 
Pagina Informazioni sull'FQDN singolo del server perimetrale
  
Per informazioni sulla distribuzione di Edge, vedere [distribuire Edge Server in Skype for Business server 2015](../../deploy/deploy-edge-server/deploy-edge-server.md)
  

