...
Si configurano gli archivi di SQL Server di backup che forniranno database di backup per il server di chat persistente o il pool di server di chat persistente
  
 **SQL Server Store**: selezionare un server SQL esistente e, facoltativamente, un'istanza per la chat persistente.
  
Fare clic su **nuovo** per definire un nuovo SQL Server e facoltativamente una nuova istanza per i dati di backup della chat persistente.
...
Fare clic su **nuovo** per definire una nuova testimonianza di SQL Server facoltativamente un'istanza per il mirroring del backup della chat persistente di SQL Server.
