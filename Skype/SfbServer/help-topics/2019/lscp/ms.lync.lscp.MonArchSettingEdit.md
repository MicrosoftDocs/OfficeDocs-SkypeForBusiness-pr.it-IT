...
- **Integrazione di Microsoft Exchange** Selezionare questa opzione se sono presenti utenti ospitati in Exchange. Con questa opzione, Exchange viene utilizzato per archiviare i dati per tali utenti, se le loro cassette postali sono state In-Place conservazione. Se tutti gli utenti sono ospitati in Exchange, non è necessario configurare database SQL Server separati per l'archiviazione dei dati di archiviazione.
