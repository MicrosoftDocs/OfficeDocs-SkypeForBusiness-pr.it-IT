...
Seguire questa procedura per configurare un trunk con bypass multimediale abilitato. Per configurare un trunk con bypass multimediale disabilitato, vedere Configurare un [trunk senza bypass multimediale in Skype for Business Server.](configure-trunk-without-media-bypass.md)

La funzionalità bypass multimediale è utile se si desidera ridurre il numero dei Mediation Server distribuiti. Per ulteriori informazioni, vedere [Pianificare il bypass multimediale in Skype for Business](../../plan-your-deployment/enterprise-voice-solution/media-bypass.md)
...
> [!NOTE]
> Il bypass multimediale non interagisce con tutti i gateway PSTN (Public Switched Telephone Network), IP-PBX e Session Border Controller (SBC). Microsoft ha testato un set di gateway PSTN e SBC con partner certificati. Il bypass multimediale è supportato solo con i prodotti e le versioni elencati nella pagina [Infrastruttura telefonica per Skype for Business Server.](https://docs.microsoft.com/SkypeForBusiness/certification/infra-gateways)
...
4. Specificare un valore in **Numero massimo di finestre di dialogo iniziali supportate.** Questo è il numero massimo di risposte in forked che un gateway PSTN (Public Switched Telephone Network), IP-PBX o SBC (Session Border Controller) ITSP può ricevere a un INVITE inviato al Mediation Server. Il valore predefinito è 20.
...
8. Se il peer trunk supporta la ricezione di richieste SIP REFER dal Mediation Server, selezionare la casella di controllo Abilita invio **riferimento al gateway.**
...
10. Organizzare i record utilizzo PSTN in modo da ottenere prestazioni ottimali. Per modificare la posizione di un record nell'elenco, selezionare il record utilizzo PSTN e fare clic sulle frecce su o giù.
...
11. **L'abilitazione del latch RTP** deve essere selezionata per abilitare i supporti di bypass per i client dietro un firewall o NAT (Network Address Translation) e un SBC che supporta il latch.

12. **L'opzione Abilita cronologia chiamate** di inoltro deve essere selezionata per consentire l'invio di informazioni sulla cronologia delle chiamate al peer gateway del Mediation Server.

13. Abilitare i dati **forward P-Asserted-Identity** per consentire l'inoltro delle informazioni sull'autore della chiamata PAI (P-Asserted-Identity) tra il lato Mediation Server e il lato gateway (e viceversa), se presenti.
