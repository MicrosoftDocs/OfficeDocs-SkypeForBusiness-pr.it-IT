...
description: 'Questo articolo illustra i passaggi per configurare un''installazione di Skype for Business Server esistente per l''utilizzo del servizio per dispositivi mobili, consentendo ai dispositivi mobili di sfruttare le funzionalità di Skype for Business Server Mobility.'
---
...
Questo articolo illustra i passaggi per configurare un'installazione di Skype for Business Server esistente per l'utilizzo del servizio per dispositivi mobili, consentendo ai dispositivi mobili di sfruttare le funzionalità di Skype for Business Server Mobility.
...
2. Fare **clic** sul pulsante Start, scegliere Strumenti di **amministrazione (potrebbe** essere necessario cercarlo se non è un'opzione fuori dal menu Start), quindi fare clic su **DNS** per aprire lo snap-in di amministrazione DNS.
...
6. Successivamente, deve essere presente un'area da immettere in un indirizzo **IP**, che deve essere l'INDIRIZZO IP per il pool Front End (o un singolo Front End Server o pool di server Director o Director), identificato nel passaggio 3 precedente.
...
7. Sarà inoltre necessario controllare le **voci Nome** soggetto alternativo. Sono disponibili una o più delle opzioni seguenti:
   ...
   - Nomi esterni di servizi Web interni e di servizi Web (ad esempio, webpool01.contoso.net, webpool01.contoso.com), in base alle scelte effettuate in Generatore di topologie e nelle selezioni di servizi Web sovraevassate.
...
3. In genere, è necessario immettere le informazioni seguenti:
   ...
   - **Azione regola**: in questo caso si tratta di una regola **Consenti** , si sta consentendo a qualcosa di passare attraverso il proxy inverso.
    
   - La **regola di** pubblicazione o l'opzione che si sta scegliendo è **un singolo sito Web o un servizio di bilanciamento del carico**.
...
3. In genere, è necessario immettere le informazioni seguenti:
   ...
   - Sarà necessario pubblicare un percorso per La pubblicazione interna e immettere il nome di dominio completo per l'indirizzo **VIP** del servizio di bilanciamento del carico del pool Front End, ad esempio sfb_pool01.contoso.local.
...
1. Eseguire l'accesso come membro del **ruolo CsAdministrator** in qualsiasi computer in **cui Skype for Business Server Management Shell** e **Ocscore** siano installati.
...
Per esaminare ulteriormente le procedure di comando, è possibile consultare [Test-CsUcwaConference](/powershell/module/skype/test-csucwaconference) e [Test-CsMcxP2PIM](/powershell/module/skype/test-csmcxp2pim).
...
> [!NOTE]
> Gli utenti che non sono abilitati per VoIP aziendale possono usare i propri dispositivi mobili per effettuare Skype chiamate VoIP Skype o partecipare alle conferenze utilizzando il collegamento Fai clic per partecipare mentre sono nei dispositivi mobili, se sono impostate le opzioni appropriate per il criterio vocale a cui sono associati. Nell'argomento PIANIFICAZIONE sono disponibili ulteriori dettagli. 
...
3. Disattivare l'accesso a Mobilità e Chiamata tramite Lavoro a livello globale digitando:
   ...
   ```
    ...
    Per altre info, vedere [Set-CsMobilityPolicy](/powershell/module/skype/set-csmobilitypolicy).
...
3. Puoi creare criteri a livello di sito, disattivare VoIP e video, abilitare Richiedi WiFi per ip audio e Richiedi WiFi per IP Video per sito. Digitare:
   ...
   ```
    ...
    Per ulteriori informazioni [, vedere New-CsMobilityPolicy](/powershell/module/skype/new-csmobilitypolicy).
