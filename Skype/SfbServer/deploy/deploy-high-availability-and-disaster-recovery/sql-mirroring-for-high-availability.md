...
Per SQL procedure consigliate in termini di versioni SQL supportate per un ruolo di controllo, vedere [Database Mirroring Witness](/sql/database-engine/database-mirroring/database-mirroring-witness).
...
6. Dopo aver definito il pool Front End e tutti gli altri ruoli della topologia, utilizzare Generatore di topologie per pubblicare la topologia. Quando la topologia viene pubblicata, se nel pool Front End che ospita l'archivio di gestione centrale è abilitato il mirroring di SQL, verrà visualizzata un'opzione per creare database dell'archivio SQL primario e SQL mirror.
...
- Le porte già allocate ad altre applicazioni sullo stesso server, comprese quelle allocate ad altre istanze di SQL, non devono essere utilizzate per le nuove istanze di SQL installate. Pertanto, se si ha più di una istanza SQL installata sullo stesso server, queste devono utilizzare porte differenti per il mirroring. Per ulteriori dettagli, vedere:
  ...
  - [Specificare un indirizzo di rete del server (mirroring del database)](/sql/database-engine/database-mirroring/specify-a-server-network-address-database-mirroring)
...
## <a name="using-skype-for-business-server-2015-management-shell-cmdlets-to-set-up-sql-mirroring"></a>Utilizzo Skype for Business Server 2015 Management Shell per configurare SQL mirroring
...
1. Aprire una Skype for Business Server 2015 Management Shell ed eseguire il cmdlet seguente:
