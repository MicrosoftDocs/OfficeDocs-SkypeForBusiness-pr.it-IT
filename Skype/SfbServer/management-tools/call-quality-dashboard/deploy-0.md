...
- **Database di archiviazione**, in cui i dati QoE (Quality of Experience) vengono replicati e archiviati.
...
La distribuzione del cubo ottiene informazioni dall'utente in cui si trova l'archivio QoE, distribuisce il cubo e imposta un normale processo agente di SQL Server che aggiorna il cubo a intervalli regolari.
...
1. Copiare il CallQualityDashboard.msi nel computer in cui deve essere installato il componente del database di archiviazione di CQD (questo è il computer SQL Server installato). 
...
6. Nella pagina Configurazione archivio QoE fornire le informazioni seguenti:
   ...
   - **Istanza SQL Server archivio QoE: un** nome di istanza SQL Server locale per il quale deve essere creato il database di archiviazione. Per utilizzare un'SQL Server predefinita, lasciare vuoto questo campo. Per usare un'SQL Server denominata, specificare il nome dell'istanza (ad esempio, il nome dopo "\").
   ...
   - **SQL Agent Job User - User Name &amp; Password:** nome account del servizio di dominio e password (mascherati) che verranno utilizzati per eseguire il passaggio "QoE Archive Data" del processo agente di SQL Server (che eseguirà la stored procedure per recuperare i dati da QoE Metrics DB in Archive DB, quindi questo account deve disporre dell'accesso in lettura al database delle metriche QoE, come indicato nella sezione Accounts). Questo account deve inoltre disporre di un account di accesso nell'SQL Server QoE Archive).
...
8. Nella pagina Configurazione cubo fornire le informazioni seguenti:
   ...
   - **Cube Analysis Server: SQL Server'istanza** di Analysis Services per la quale deve essere creato il cubo. Può trattarsi di un computer diverso, ma l'utente che installa deve essere membro degli amministratori del server dell'istanza di Analysis Service di SQL Server di destinazione.
...
2. Espandi **l'oggetto QoECube** , seleziona **Metrica QoE**, fai clic con il pulsante destro del mouse e quindi scegli **Sfoglia**. 
...
Assegnagli la data del primo record (identifica che usando il comando select MIN(StartTime) FROM CqdPartitionedStreamView SQL ), un valore EndDate di domani e quindi NULL per gli ultimi due valori.
