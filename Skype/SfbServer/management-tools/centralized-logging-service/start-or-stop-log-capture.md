---
title: Avviare o arrestare l'acquisizione del log CLS in Skype for Business Server 2015
ms.reviewer: ''
ms.author: v-cichur
author: cichur
manager: serdars
...
description: 'Riepilogo: informazioni su come avviare o arrestare una sessione di acquisizione del registro del servizio di registrazione centralizzata in Skype for Business Server 2015.'
...
2. Sono disponibili ulteriori parametri per la gestione dei comandi di registrazione. È possibile utilizzare -Duration per modificare l'intervallo di tempo per l'esecuzione dello scenario. È inoltre possibile definire -Computers, un elenco di nomi di dominio completi (FQDN) dei computer separati da una virgola oppure -Pools, un elenco delimitato da virgole di FQDN per i pool su cui si desidera eseguire l'accesso.
...
> [!NOTE]
> Pertanto, ci si potrebbe chiedere: Ora che è stata abilitata la registrazione, dove vengono conservati i log? Poiché si accederà alle informazioni archiviate nei registri utilizzando query della shell di gestione inviate agli agenti CLS, è possibile inviare i risultati in diversi formati di file possibili, in cui in ogni server un agente CLS mantiene i propri record non è effettivamente importante da conoscere.  I file di registro possono essere salvati in un percorso specificato e letto e analizzato utilizzando una vasta gamma di strumenti, tra cuiSnooper.exee qualsiasi strumento **in** grado di leggere un file di testo, ad esempio **Notepad.exe**. Snooper.exe fa parte degli strumenti di debug di Skype for Business Server 2015 ed è disponibile come [download Web.](https://go.microsoft.com/fwlink/p/?LinkId=285257)
