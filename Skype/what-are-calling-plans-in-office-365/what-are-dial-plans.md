...
Ambito di un dial plan determina il livello della gerarchia in cui è possibile applicare il dial plan. Gli ambiti sono diversi da quella in Skype per Business Server 2015 distribuzione locale. I client ottengono appropriati del dial plan tramite il provisioning delle impostazioni che sono fornite automaticamente quando gli utenti accedono a Skype Business online. In qualità di amministratore, è possibile gestire e assegnare livelli di ambito del dial plan utilizzando PowerShell remoto.
  
In Skype Business online, esistono due tipi di plan di messaggistica unificata-tenant, ovvero per l'organizzazione, l'ambito e nell'ambito del servizio. Un dial plan con ambito servizio è definito per ogni paese/area geografica in cui è disponibile il sistema telefonico di Office 365. Ogni utente viene automaticamente assegnato il dial plan paese servizio che genera una corrispondenza per il percorso di utilizzo Office 365 assegnati all'utente. Non è possibile modificare il dial plan paese servizio, ma è possibile creare i dial plan tenant con ambito, quale aggiungere il dial plan paese servizio. Come i client sono predisposti, ottengono un "efficace dial plan," che corrisponde a una combinazione di servizio paese dial plan e il dial plan tenant con ambito in modo appropriato. Non è pertanto necessario definire tutte le regole di normalizzazione nel dial plan di tenant poiché potrebbe essere già esistenti nel servizio paese dial plan.
...
- **Passaggio 2** Determinare se sono necessari tenant globale o tenant con ambito utente dial plan o entrambi. I dial plan con ambito utente sono necessarie se gli utenti hanno i requisiti di composizione locale diverso.
...
Per i dial plan utente, è necessario specificare un nome descrittivo che identifica il dial plan agli utenti verrà assegnato. Il dial plan nome semplice è prepopolato con un valore string che dipende dal nome del dial plan. Il campo nome semplice è modificabile, che consente di creare una convenzione di denominazione più descrittiva per il dial plan. Il valore nome semplice non può essere vuoto e deve essere univoco. È consigliabile sviluppare una convenzione di denominazione per tutta l'organizzazione e quindi utilizzarla in modo coerente questa convention su tutti i siti e utenti.
...
Uno o più regole di normalizzazione devono essere assegnate al dial plan. Le regole di normalizzazione vengono eseguite dall'alto verso il basso, in modo che l'ordine in cui vengono visualizzati in un tenant dial plan è importante. Ad esempio, se un tenant dial plan è 10 regole di normalizzazione, la logica di corrispondenza numerica composto verrà tentata inizia con la prima regola di normalizzazione, se non vi è una corrispondenza quindi il secondo e così via. Se viene individuata una corrispondenza, viene utilizzata tale regola e non esiste alcun impegno da soddisfare tutte le altre regole definite. Può esistere un massimo di 25 regole di normalizzazione in un determinato tenant dial plan.
...
|Non tradotto  <br/> |Abbinamento a 5 cifre senza conversione.  <br/> |^(\\d{5})$  <br/> |$1  <br/> |34567 viene convertito in 34567  <br/> |
