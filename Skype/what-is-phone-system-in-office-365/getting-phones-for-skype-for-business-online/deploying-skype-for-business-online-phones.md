...
Skype per Business Online supportato telefono e firmware è compatibile per Skype per Business Server anche, ma l'opposto non è sempre true. Per verificare che si desidera acquistare o il provisioning di un telefono supportato e firmware, vedere [Getting telefoni Skype Business online](getting-phones-for-skype-for-business-online.md).
...
- **Telefoni AudioCodes**, Vai a **dal Menu** > **Stato dispositivo** > **versione del Firmware** nella schermata start.
...
Gli aggiornamenti del firmware vengono gestiti dal Skype per servizio di Business. Ogni Skype per le aziende Certificate firmware del telefono viene caricato in Skype per aggiornare Business server e dell'aggiornamento dei dispositivi è abilitata in tutti i telefoni per impostazione predefinita. In base al tempo di inattività sul telefono e intervalli di polling, telefoni verranno automaticamente scaricare e installare l'ultima build Certificate. È possibile disabilitare le impostazioni di aggiornamento dispositivo utilizzando il cmdlet [Set-CsIPPhonePolicy](https://technet.microsoft.com/en-us/library/mt629497.aspx) e l'impostazione del parametro _EnableDeviceUpdate_ `false`.
...
- **Blocco telefono** è una funzionalità di recente introdotta in Skype per le aziende Certificate telefoni che consente di proteggere un telefono. Se attivata, gli utenti verranno richiesto di creare un PIN dopo un'autenticazione eseguita correttamente. Una volta creato, telefoni consente di bloccare il timeout inattività definite scade, un utente blocca manualmente il proprio telefono o la sincronizzazione con il blocco del telefono con il blocco di PC utilizzando l'abbinamento telefono. Se blocco telefono viene immesso il PIN errato più volte, il telefono verrà disconnesso l'utente o richiedono il codice dell'amministratore di sbloccare il telefono, ma questa variano a seconda del partner telefono. Il PIN dell'utente deve essere compresa tra 6 e 15 cifre.
...
BToE può essere configurato per operare in due modalità: *automatico* (impostazione predefinita) e *manuale* . Può inoltre essere attivato (impostazione predefinita) / disabilitata per gli utenti che utilizzano Skype per le impostazioni in-band aziendale. Per operare in modalità *manuale* , sarà necessario eseguire un passaggio aggiuntivo associare il telefono con le app di Windows.
...
[Ecco cosa viene visualizzato con sistema telefonico in Office 365](../../what-is-phone-system-in-office-365/here-s-what-you-get-with-phone-system.md)
