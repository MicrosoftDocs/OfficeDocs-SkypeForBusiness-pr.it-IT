...
Il consolidamento di tutti gli utenti provenienti da locali nel cloud in una singola organizzazione di Microsoft 365 o Office 365 può essere raggiunto per qualsiasi organizzazione con più distribuzioni di Skype for business, purché vengano soddisfatti i requisiti chiave seguenti:

- Sono presenti al massimo un'organizzazione di Microsoft 365 o Office 365. Il consolidamento in scenari con più di un'organizzazione non è supportato.
...
2.  Verificare che tutti i domini SIP rilevanti nelle distribuzioni locali siano verificati Microsoft 365 o i domini di Office 365.
3.  Scegliere una distribuzione Skype for business che sarà ibrida con Microsoft 365 o Office 365. In questo esempio verrà utilizzato OriginalCompany. <span> com.
...
10. Dopo che tutti gli utenti sono stati spostati nel cloud, [disabilitare l'ambiente ibrido con la distribuzione locale di Skype for business](cloud-consolidation-disabling-hybrid.md) per OriginalCompany. <span> com da Office 365:  
    - Disabilitare il dominio diviso nell'organizzazione Microsoft 365 o Office 365.
...
11. Se non è già stato fatto, [abilitare AAD Connect per la foresta successiva](cloud-consolidation-aad-connect.md) che passerà a un ambiente ibrido (AcquiredCompany. <span> com). A questo punto, l'organizzazione ha l'aspetto di **[Figura C](#figure-c)**. Può trattarsi di un altro punto di partenza comune per alcune organizzazioni. 
12. In PowerShell, [abilitare i domini SIP per la successiva distribuzione locale](https://docs.microsoft.com/powershell/module/skype/enable-csonlinesipdomain) che andrà a un ambiente ibrido, AcquiredCompany. <span> com. Questa operazione viene fatta usando `Enable-CsOnlineSipDomain` , che è una nuova funzionalità disponibile al 2018 dicembre.
13. Se si utilizza la federazione chiusa, è necessario aggiungere qualsiasi dominio SIP ( \* ad eccezione di microsoftonline.com) del tenant puro online come domini consentiti nello **stesso** Microsoft 365 o Office 365. Si noti che è possibile richiedere un certo tempo prima che la modifica abbia effetto e che non vi siano danni in questo modo in anticipo, quindi si consiglia di procedere bene prima di passare al passaggio 14.
...
- Lo spazio degli indirizzi SIP condiviso è abilitato tra AcquiredCompany. <span> com e Microsoft 365 o l'organizzazione di Office 365.
...
I passaggi riportati nell'esempio canonico precedente presuppongono che l'organizzazione inizi con due distribuzioni locali federate con nessuna presenza Microsoft o Office 365. Tuttavia, alcune organizzazioni possono disporre di un footprint di Microsoft 365 o di Office 365 esistente e possono essere presenti diversi punti di ingresso nella sequenza precedente. Sono disponibili quattro configurazioni tipiche:

- Più organizzazioni locali federate con nessuna organizzazione Microsoft 365 o Office 365. In questo caso, iniziare dal passaggio 1.
...
- Sono presenti al massimo un'organizzazione di Microsoft 365 o Office 365. Il consolidamento in scenari con più di un'organizzazione non è supportato.
- Solo una foresta Skype for business locale può essere in modalità ibrida (spazio di indirizzi SIP condiviso) alla volta. Tutte le altre foreste di Skype for business locali devono rimanere solo in locale e devono essere federate tra loro e con l'organizzazione Microsoft 365 o Office 365.
