...
In questo argomento vengono descritti i requisiti di infrastruttura e di sistema necessari per configurare la connettività ibrida tra la distribuzione locale Skype for Business Server locale e Teams.
...
> - Non sarà più possibile assegnare una modalità diversa da TeamsOnly a un utente che si trova online.
...
A seconda della posizione del data center di Microsoft Online Services, è inoltre necessario configurare i dispositivi firewall di rete in modo che accettino connessioni basate su nomi di dominio con caratteri jolly(ad esempio, tutto il traffico \* da .outlook.com). Se i firewall dell'organizzazione non supportano le configurazioni dei nomi con caratteri jolly, sarà necessario determinare manualmente gli intervalli di indirizzi IP che si desidera consentire e le porte specificate.
