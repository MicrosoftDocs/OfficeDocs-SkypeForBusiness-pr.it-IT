...
- Strumenti di amministrazione online. È possibile utilizzare l'interfaccia Teams o Windows PowerShell per gestire Teams. Per usare PowerShell per gestire Teams, scaricare e installare il modulo Teams PowerShell. (Il Skype for Business Online Connector è stato ritirato).
...
- **Più distribuzioni di Skype for Business Server in più foreste.** Questa configurazione può derivare da scenari di fusione e acquisizione, nonché in aziende più complesse. Il consolidamento di tutti gli utenti dall'ambiente locale al cloud in una singola organizzazione Microsoft 365 può essere raggiunto per qualsiasi organizzazione con più distribuzioni Skype for Business, purché siano soddisfatti i requisiti chiave seguenti:
