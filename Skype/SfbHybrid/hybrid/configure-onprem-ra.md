...
Se l Sistema telefonico operatore automatico o la coda di chiamata necessitano di un numero di servizio, le varie dipendenze possono essere soddisfatte nella sequenza seguente:

1. Ottenere un numero di servizio.
2. Ottenere una licenza Sistema telefonico - Licenza utente [virtuale](/MicrosoftTeams/teams-add-on-licensing/virtual-user) o una licenza Sistema telefonico a pagamento da usare con l'account della risorsa.
...
Per reindirizzare le chiamate agli utenti dell'organizzazione ospitati **online,** devono disporre di una licenza di Sistema telefonico ed essere abilitati per VoIP aziendale o disporre di piani di chiamata Microsoft 365 o Office 365 chiamata. Vedi [Assegnare Microsoft Teams licenze](/MicrosoftTeams/assign-teams-licenses). Per abilitarli per VoIP aziendale, è possibile usare Windows PowerShell. Ad esempio, eseguire:  `Set-CsUser -identity "Amos Marble" -EnterpriseVoiceEnabled $true`
...
4. Associare l'account della risorsa a Sistema telefonico operatore automatico o coda di chiamata
...
3. Creare un account di risorsa locale eseguendo il cmdlet per ogni operatore automatico o coda di chiamata Sistema telefonico e assegnare a ognuno un nome, un indirizzo `New-CsHybridApplicationEndpoint` SIP e così via.
...
5. Assegnare la Sistema telefonico - Utente virtuale o Sistema telefonico licenza all'account della risorsa. Vedi [Assegnare Microsoft Teams licenze dei componenti aggiuntivi e](/MicrosoftTeams/teams-add-on-licensing/assign-teams-add-on-licenses) [Assegnare licenze agli utenti.](/microsoft-365/admin/manage/assign-licenses-to-users)
...
8. Associare l'account della risorsa all'Sistema telefonico automatico o alla coda di chiamata scelta in precedenza.
...
Questi passaggi sono necessari in Sistema telefonico dipendentemente dal fatto che si crei un operatore automatico o una struttura della coda di chiamata nuova o che si ricompila la struttura originariamente creata Exchange messaggistica unificata.

Accedere al server Skype for Business front-end ed eseguire i cmdlet di PowerShell seguenti:

1. Creare un account di risorsa locale eseguendo il cmdlet per ogni operatore automatico o coda di chiamata Sistema telefonico e assegnare a ognuno un nome, un indirizzo `New-CsHybridApplicationEndpoint` SIP e così via.
...
3. Creare l'Sistema telefonico operatore automatico o la coda di chiamata. Visualizzare uno tra:
   - [Configurare un operatore automatico cloud](/MicrosoftTeams/create-a-phone-system-auto-attendant)
   - [Creare code delle chiamate nel cloud](/MicrosoftTeams/create-a-phone-system-call-queue)  
4. Associare l'account della risorsa e l Sistema telefonico operatore automatico o la coda di chiamata scelto in precedenza.
...
La migrazione da Exchange messaggistica unificata a Sistema telefonico richiederà la ricreazione della struttura della coda di chiamata e dell'operatore automatico, la migrazione diretta da uno all'altro non è supportata. Per implementare di nuovo un set di code di chiamata e operatori automatici:
...
2. Per ogni coda di chiamata o operatore automatico di messaggistica unificata elencato, prendere nota della relativa posizione nella struttura, nelle impostazioni e ottenere copie dei file audio o di sintesi vocale associati (il guid nell'output sarà il nome di una cartella in cui sono archiviati i file). Exchange È possibile ottenere questi dettagli eseguendo il comando:
...
6. Riassegnare il numero di telefono collegato alla Exchange o all'operatore automatico di messaggistica unificata all'operatore Sistema telefonico o alla coda di chiamata corrispondente.  
