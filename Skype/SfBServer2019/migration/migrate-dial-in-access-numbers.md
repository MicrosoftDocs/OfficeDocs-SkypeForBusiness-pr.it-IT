...
2. Per spostare ogni numero di accesso remoto in un pool ospitato Skype for Business Server 2019, dalla riga di comando eseguire: 

   ```PowerShell
   Move-CsApplicationEndpoint -Identity <SIP URI of the access number to be moved> -Target <FQDN of the pool to which the access number is moving>
   ```

3. Aprire Skype for Business Server Pannello di controllo.
...
6. Verificare che non rimangano numeri di accesso esterno per il pool di installazione legacy da cui si sta eseguendo la migrazione.
...
## <a name="verify-the-dial-in-access-number-migration-using-skype-for-business-server-control-panel"></a>Verificare la migrazione dei numeri di accesso remoto tramite Skype for Business Server pannello di controllo
