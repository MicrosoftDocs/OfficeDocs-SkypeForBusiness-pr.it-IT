...
1.  Quando il modello richiesto è disponibile per la CA, utilizzare il cmdlet di Windows PowerShell seguente dal server perimetrale per richiedere il certificato:
    ...
    Il nome del certificato predefinito del modello utilizzato per Lync Server è il server Web. Specificare solo \<template name\> se è necessario utilizzare un modello diverso dal modello predefinito.
