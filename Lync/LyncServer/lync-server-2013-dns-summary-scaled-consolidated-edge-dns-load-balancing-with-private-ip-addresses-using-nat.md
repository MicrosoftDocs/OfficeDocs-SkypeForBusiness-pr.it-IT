...
# <a name="dns-summary---scaled-consolidated-edge-dns-load-balancing-with-private-ip-addresses-using-nat-in-lync-server-2013"></a>Server perimetrale consolidato in scala di riepilogo DNS, bilanciamento del carico DNS con indirizzi IP privati tramite NAT in Lync 2013
...
Per evitare problemi di routing, verificare che siano presenti almeno due schede di rete nei server perimetrali e che il gateway predefinito sia impostato solo sulla scheda di rete associata all'interfaccia esterna. Ad esempio, come illustrato nella figura dello scenario perimetrale consolidato in scala in scala [consolidata, bilanciamento del carico DNS con indirizzi IP privati tramite NAT in Lync Server 2013](lync-server-2013-scaled-consolidated-edge-dns-load-balancing-with-private-ip-addresses-using-nat.md), il gateway predefinito dovrebbe puntare al firewall esterno.
...
È possibile configurare due schede di rete in ogni server perimetrale, come illustrato di seguito:
  ...
  - **Scheda di rete 2 - Nodo 1 (interfaccia esterna)**
    ...
    > [!NOTE]  
    > È possibile, anche se non consigliabile, utilizzare un unico indirizzo IP per tutte e tre le interfacce di servizio Edge. Sebbene consenta di ridurre il numero di indirizzi IP, questa soluzione richiede numeri di porta diversi per ogni servizio. Il numero di porta predefinito è 443/TCP, che assicura che il traffico sia consentito dalla maggior parte dei firewall remoti. Se i valori delle porte vengono impostati, ad esempio, su 5061/TCP per Access Edge, 444/TCP per Web Conferencing Edge e 443/TCP per A/V Edge, è possibile che gli utenti remoti riscontrino problemi se il firewall da cui sono protetti non consente il traffico sulle porte 5061/TCP e 444/TCP. Inoltre, tre indirizzi IP distinti semplificano le attività di risoluzione dei problemi grazie alla possibilità di filtrare in base all'indirizzo IP.
...
### <a name="dns-records-required-for-scaled-consolidated-edge-dns-load-balancing-with-private-ip-addresses-using-nat-example"></a>Record DNS necessari per server perimetrale consolidato in scala, servizio di bilanciamento del carico DNS con indirizzi IP privati tramite NAT (esempio)
...
<td><p>Interfaccia esterna Access Edge (Contoso). Ripetere per tutti i domini SIP con utenti abilitati per Lync, se necessario</p></td>
</tr>
<tr class="even">
<td><p>DNS esterno/A</p></td>
<td><p>webcon.contoso.com</p></td>
<td><p>131.107.155.20 e 131.107.155.21</p></td>
<td><p>Interfaccia esterna Web Conferencing Edge</p></td>
</tr>
<tr class="odd">
<td><p>DNS esterno/A</p></td>
<td><p>av.contoso.com</p></td>
<td><p>131.107.155.30 e 131.107.155.31</p></td>
<td><p>Interfaccia esterna A/V Edge</p></td>
</tr>
<tr class="even">
...
## <a name="dns-summary--public-instant-messaging-connectivity"></a>Riepilogo DNS – Public Instant Messaging Connectivity
...
## <a name="dns-summary-for-extensible-messaging-and-presence-protocol"></a>Riepilogo DNS per Extensible Messaging and Presence Protocol
