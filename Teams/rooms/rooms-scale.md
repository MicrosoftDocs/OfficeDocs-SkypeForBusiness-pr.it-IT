...
### <a name="networking-requirements"></a><span data-ttu-id="17e77-123">Requisiti di rete</span><span class="sxs-lookup"><span data-stu-id="17e77-123">Networking requirements</span></span>
...
1.  <span data-ttu-id="17e77-143">Nella console di Configuration Manager accedere ai **punti di distribuzione**di **Amministrazione** \> .</span><span class="sxs-lookup"><span data-stu-id="17e77-143">In the Configuration Manager console, go to **Administration** \> **Distribution Points**.</span></span>
...
### <a name="configure-a-boot-image"></a><span data-ttu-id="17e77-159">Configurare un'immagine di avvio</span><span class="sxs-lookup"><span data-stu-id="17e77-159">Configure a boot image</span></span>
...
### <a name="create-folders-for-the-package-source-files"></a><span data-ttu-id="17e77-219">Creare cartelle per i file di origine del pacchetto</span><span class="sxs-lookup"><span data-stu-id="17e77-219">Create folders for the package source files</span></span>
...
### <a name="create-the-monitoring-agent-package"></a><span data-ttu-id="17e77-235">Creare il pacchetto agente di monitoraggio</span><span class="sxs-lookup"><span data-stu-id="17e77-235">Create the Monitoring agent package</span></span>
...
6. <span data-ttu-id="17e77-260">Selezionare non **creare un programma**e quindi fare clic su **Avanti**.</span><span class="sxs-lookup"><span data-stu-id="17e77-260">Select **Do not create a program**, and then select **Next**.</span></span>
...
4.  <span data-ttu-id="17e77-275">Selezionare non **creare un programma**e quindi fare clic su **Avanti**.</span><span class="sxs-lookup"><span data-stu-id="17e77-275">Select **Do not create a program**, and then select **Next**.</span></span>
...
6.  <span data-ttu-id="17e77-288">Esaminare la pagina **confermare le impostazioni** e quindi scegliere **Avanti**.</span><span class="sxs-lookup"><span data-stu-id="17e77-288">Review the **Confirm the settings** page, and then select **Next**.</span></span>
...
4. <span data-ttu-id="17e77-308">Immettere le informazioni seguenti per creare il pacchetto:</span><span class="sxs-lookup"><span data-stu-id="17e77-308">Enter the following information to create the package:</span></span>
...
6.  <span data-ttu-id="17e77-322">Selezionare **Chiudi**.</span><span class="sxs-lookup"><span data-stu-id="17e77-322">Select **Close**.</span></span>
...
4.  <span data-ttu-id="17e77-335">Selezionare **Importa tutti i driver nel percorso di rete seguente (UNC)**, selezionare la cartella di origine, ad esempio C\\:\\_Sources\\driver Surface Pro, quindi selezionare **Avanti**.</span><span class="sxs-lookup"><span data-stu-id="17e77-335">Select **Import all drivers in the following network path (UNC)**, select the source folder (for example, C:\\_Sources\\Drivers\\Surface Pro), and then select **Next**.</span></span>
...
3.  <span data-ttu-id="17e77-371">Distribuire pacchetti di sistemi operativi.</span><span class="sxs-lookup"><span data-stu-id="17e77-371">Distribute operating system packages.</span></span>
    ...
    2.  <span data-ttu-id="17e77-374">Esaminare l'elenco dei pacchetti e quindi selezionare **Avanti**.</span><span class="sxs-lookup"><span data-stu-id="17e77-374">Review the list of packages, and then select **Next**.</span></span>
...
### <a name="import-the-sample-task-sequence"></a><span data-ttu-id="17e77-383">Importare la sequenza di attività di esempio</span><span class="sxs-lookup"><span data-stu-id="17e77-383">Import the sample task sequence</span></span>
...
2. <span data-ttu-id="17e77-394">Scorrere ogni passaggio e completare gli aggiornamenti consigliati:</span><span class="sxs-lookup"><span data-stu-id="17e77-394">Walk through each step and complete the recommended updates:</span></span>
   ...
   6. <span data-ttu-id="17e77-409">**Applica impostazioni di rete**: questo passaggio consente di specificare un nome di dominio e un'unità organizzativa di gruppo di lavoro o Active Directory.</span><span class="sxs-lookup"><span data-stu-id="17e77-409">**Apply Network Settings**: This step allows you to specify a workgroup or Active Directory domain name and organizational unit.</span></span>
      > [!NOTE]
      > <span data-ttu-id="17e77-410">Vedi [considerazioni sul dominio del sistema room di Skype](domain-joining-considerations.md) per le azioni consigliate necessarie per la distribuzione delle unità di Microsoft teams rooms come membri di un dominio della directory actve.</span><span class="sxs-lookup"><span data-stu-id="17e77-410">See [Skype Room System domain joining considerations](domain-joining-considerations.md) for recommended actions you need to take in deploying Microsoft Teams Rooms units as members of an Actve Directory domain.</span></span>
   7. <span data-ttu-id="17e77-411">**Applicare i driver:** Questo passaggio e i relativi passaggi secondari vengono usati per distribuire i driver di dispositivo e il firmware applicabili in base al modello Surface Pro in uso.</span><span class="sxs-lookup"><span data-stu-id="17e77-411">**Apply Drivers:** This step and its sub-steps are used to deploy applicable device drivers and firmware based on the Surface Pro model you have.</span></span> <span data-ttu-id="17e77-412">Aggiornare ogni passaggio per specificare il pacchetto del driver pertinente associato alla distribuzione.</span><span class="sxs-lookup"><span data-stu-id="17e77-412">Update each step to specify the relevant driver package associated with this deployment.</span></span>
      -   <span data-ttu-id="17e77-413">Ogni pacchetto di driver è configurato per sfruttare i filtri di Strumentazione gestione Windows (WMI) per distribuire driver e firmware rilevanti in base alla creazione e al modello di Surface Pro.</span><span class="sxs-lookup"><span data-stu-id="17e77-413">Each driver package is configured to leverage Windows Management Instrumentation (WMI) filters to deploy relevant drivers and firmware based on the Surface Pro make and model.</span></span>
      -   <span data-ttu-id="17e77-414">Ti consigliamo vivamente di non modificare la configurazione di questi driver, altrimenti la distribuzione potrebbe non riuscire.</span><span class="sxs-lookup"><span data-stu-id="17e77-414">We highly recommend that you not alter the configuration of these drivers, otherwise deployment might fail.</span></span>

   8. <span data-ttu-id="17e77-415">Configurare **Windows e Configuration Manager**: questo passaggio consente di distribuire e configurare il client di Configuration Manager.</span><span class="sxs-lookup"><span data-stu-id="17e77-415">**Set up Windows and Configuration Manager**: This step deploys and configures the Configuration Manager client.</span></span> <span data-ttu-id="17e77-416">Aggiornare questo passaggio per specificare il pacchetto client predefinito di Configuration Manager.</span><span class="sxs-lookup"><span data-stu-id="17e77-416">Update this step to specify the built-in Configuration Manager Client Package.</span></span>

   9. <span data-ttu-id="17e77-417">**Installa certificato radice**: questo passaggio distribuisce il certificato radice per i dispositivi non collegati al dominio e quindi è facoltativo e disabilitato per impostazione predefinita.</span><span class="sxs-lookup"><span data-stu-id="17e77-417">**Install Root Certificate**: This step distributes the root certificate for non–domain-joined devices, and therefore is optional and disabled by default.</span></span>
      -   <span data-ttu-id="17e77-418">Abilitare questo passaggio se è necessario distribuire un certificato radice nelle unità di Microsoft teams rooms.</span><span class="sxs-lookup"><span data-stu-id="17e77-418">Enable this step if you need to deploy a root certificate to the Microsoft Teams Rooms units.</span></span>
      -   <span data-ttu-id="17e77-419">Se è necessario eseguire questo passaggio, verificare che il **Pacchetto SRS V2-root certificate** e **disabilitare il reindirizzamento del file System di 64 bit** siano selezionati.</span><span class="sxs-lookup"><span data-stu-id="17e77-419">If you do need to perform this step, verify that the **SRS v2 – Root Certificate Package** and **Disable 64-bit file system redirection** are selected.</span></span>

   10. <span data-ttu-id="17e77-420">**Installare e configurare agente di monitoraggio**: in questo passaggio viene installata la versione a 64 bit dell'agente di monitoraggio Microsoft Azure e viene configurato l'agente per la connessione all'area di lavoro analisi log.</span><span class="sxs-lookup"><span data-stu-id="17e77-420">**Install and Configure Monitoring Agent**: This step installs the 64-bit version of the Microsoft Azure Monitor agent and configures the agent to connect to your Log Analytics workspace.</span></span>
       -   <span data-ttu-id="17e77-421">Questo passaggio è disabilitato per impostazione predefinita.</span><span class="sxs-lookup"><span data-stu-id="17e77-421">This step is disabled by default.</span></span> <span data-ttu-id="17e77-422">Abilitare questo passaggio solo se si vuole usare l'agente di monitoraggio per monitorare l'integrità delle unità di Microsoft teams rooms.</span><span class="sxs-lookup"><span data-stu-id="17e77-422">Enable this step only if you’re going to use the Monitoring Agent to monitor the health of your Microsoft Teams Rooms units.</span></span>
       -   <span data-ttu-id="17e77-423">Modificare questo passaggio e aggiornare i parametri della riga di comando per specificare l' **ID area** di lavoro e la **chiave dell'area di lavoro**.</span><span class="sxs-lookup"><span data-stu-id="17e77-423">Edit this step and update the command-line parameters to specify your **Workspace ID** and **Workspace Key**.</span></span>
       -   <span data-ttu-id="17e77-424">Per altre informazioni su come ottenere l'ID area di lavoro di Operations Management Suite e la chiave primaria, vedere [configurare i dispositivi di test per il monitoraggio di Azure](azure-monitor-deploy.md#configure-test-devices-for-azure-monitoring) .</span><span class="sxs-lookup"><span data-stu-id="17e77-424">See [Configure test devices for Azure Monitoring](azure-monitor-deploy.md#configure-test-devices-for-azure-monitoring) for more information about obtaining the Operations Management Suite Workspace ID and the primary key.</span></span>
       -   <span data-ttu-id="17e77-425">Verificare che il **Pacchetto SRS V2-Microsoft Monitoring Agent** e **disabilitare il reindirizzamento del file System di 64 bit** siano selezionati.</span><span class="sxs-lookup"><span data-stu-id="17e77-425">Verify that the **SRS v2 – Microsoft Monitoring Agent Package** and **Disable 64-bit file system redirection** are selected.</span></span>
       -   <span data-ttu-id="17e77-426">Per altre informazioni sul monitoraggio dell'integrità della distribuzione di Microsoft teams rooms, vedere [pianificare la gestione di Microsoft teams Rooms con Azure monitor](azure-monitor-plan.md), [distribuire Gestione di Microsoft teams Rooms con Azure monitor](azure-monitor-deploy.md) e [gestire i dispositivi Microsoft teams Rooms con Azure monitor](azure-monitor-manage.md).</span><span class="sxs-lookup"><span data-stu-id="17e77-426">For more information about monitoring the health of your Microsoft Teams Rooms deployment, see [Plan Microsoft Teams Rooms management with Azure Monitor](azure-monitor-plan.md), [Deploy Microsoft Teams Rooms management with Azure Monitor](azure-monitor-deploy.md) and [Manage Microsoft Teams Rooms devices with Azure Monitor](azure-monitor-manage.md).</span></span>

   11. <span data-ttu-id="17e77-427">**Copiare i file di configurazione di SRS V2**: in questo passaggio vengono copiati i file di installazione e configurazione necessari dal kit di distribuzione di Microsoft teams Rooms nel disco rigido locale.</span><span class="sxs-lookup"><span data-stu-id="17e77-427">**Copy SRS v2 Configuration Files**: This step copies the required setup and configuration files from the Microsoft Teams Rooms deployment kit to the local hard drive.</span></span> <span data-ttu-id="17e77-428">Per questo passaggio non è necessario alcun adattamento.</span><span class="sxs-lookup"><span data-stu-id="17e77-428">No customization is required for this step.</span></span>
       -   <span data-ttu-id="17e77-429">Verificare che il **pacchetto di applicazione SRS V2-SRS** e **disabilitare il reindirizzamento del file System di 64 bit** siano selezionati.</span><span class="sxs-lookup"><span data-stu-id="17e77-429">Verify that the **SRS v2 – SRS Application Package** and **Disable 64-bit file system redirection** are selected.</span></span>

   12. <span data-ttu-id="17e77-430">**Install-SRSv2-OS-Updates**: questo passaggio distribuisce gli eventuali aggiornamenti obbligatori del sistema operativo necessari con la distribuzione di Microsoft teams rooms.</span><span class="sxs-lookup"><span data-stu-id="17e77-430">**Install-SRSv2-OS-Updates**: This step deploys any mandatory operating system updates required with the Microsoft Teams Rooms deployment.</span></span> <span data-ttu-id="17e77-431">Effettuare le seguenti operazioni:</span><span class="sxs-lookup"><span data-stu-id="17e77-431">Do the following:</span></span>
       -   <span data-ttu-id="17e77-432">Selezionare [Configura una console Microsoft teams Rooms](console.md) per vedere quali aggiornamenti sono necessari.</span><span class="sxs-lookup"><span data-stu-id="17e77-432">Check [Configure a Microsoft Teams Rooms console](console.md) to see which updates are required.</span></span>
       -   <span data-ttu-id="17e77-433">Verificare che il **pacchetto degli aggiornamenti di SRS V2-OS** includa tutti gli aggiornamenti necessari.</span><span class="sxs-lookup"><span data-stu-id="17e77-433">Verify that your **SRS v2 – OS Updates Package** includes all the required updates.</span></span>
       -   <span data-ttu-id="17e77-434">Verificare che sia selezionato il **Pacchetto SRS V2-OS Updates** .</span><span class="sxs-lookup"><span data-stu-id="17e77-434">Verify that the **SRS v2 – OS Updates Package** is selected.</span></span>
       -   <span data-ttu-id="17e77-435">Verificare che il criterio di esecuzione di PowerShell sia impostato su **bypass**.</span><span class="sxs-lookup"><span data-stu-id="17e77-435">Verify that the PowerShell execution policy is set to **Bypass**.</span></span>

   13. <span data-ttu-id="17e77-436">**Riavviare il computer**: questo passaggio riavvia il computer dopo l'installazione degli aggiornamenti obbligatori del sistema operativo.</span><span class="sxs-lookup"><span data-stu-id="17e77-436">**Restart Computer**: This step reboots the computer after the mandatory operating system updates are installed.</span></span> <span data-ttu-id="17e77-437">Per questo passaggio non è necessario alcun adattamento.</span><span class="sxs-lookup"><span data-stu-id="17e77-437">No customization is required for this step.</span></span>

   14. <span data-ttu-id="17e77-438">**Configurare i componenti di Windows**: questo passaggio Configura le funzionalità di Windows necessarie.</span><span class="sxs-lookup"><span data-stu-id="17e77-438">**Configure Windows Components**: This step configures the required Windows features.</span></span> <span data-ttu-id="17e77-439">Per questo passaggio non è necessario alcun adattamento.</span><span class="sxs-lookup"><span data-stu-id="17e77-439">No customization is required for this step.</span></span>

   15. <span data-ttu-id="17e77-440">**Riavviare il computer**: questo passaggio riavvia il computer dopo la configurazione delle funzionalità di Windows.</span><span class="sxs-lookup"><span data-stu-id="17e77-440">**Restart Computer**: This step reboots the computer after the Windows features are configured.</span></span> <span data-ttu-id="17e77-441">Per questo passaggio non è necessario alcun adattamento.</span><span class="sxs-lookup"><span data-stu-id="17e77-441">No customization is required for this step.</span></span>

   16. <span data-ttu-id="17e77-442">**Aggiungere un utente locale di Skype**: questo passaggio crea l'account Skype locale usato per accedere automaticamente a Windows e avviare l'applicazione Microsoft teams rooms.</span><span class="sxs-lookup"><span data-stu-id="17e77-442">**Add Local Skype User**: This step creates the local Skype account used to automatically sign in to Windows and start the Microsoft Teams Rooms application.</span></span> <span data-ttu-id="17e77-443">Questo passaggio non contiene alcun pacchetto software associato e non è necessario alcun adattamento.</span><span class="sxs-lookup"><span data-stu-id="17e77-443">This step doesn’t have any software package associated with it, and no customization is required for it.</span></span>

   17. <span data-ttu-id="17e77-444">**Impostare e configurare l'applicazione SRS**: in questo passaggio viene configurata l'installazione dell'applicazione Microsoft teams Rooms per l'avvio successivo del sistema operativo.</span><span class="sxs-lookup"><span data-stu-id="17e77-444">**Set up and configure SRS application**: This step configures the Microsoft Teams Rooms application installation for the next boot of the operating system.</span></span>
       -   <span data-ttu-id="17e77-445">Verificare che il **pacchetto di installazione SRS V2-configura SRS** e **disabilitare il reindirizzamento del file System di 64 bit** sia selezionato.</span><span class="sxs-lookup"><span data-stu-id="17e77-445">Verify that the **SRS v2 – Configure SRS Setup Package** and **Disable 64-bit file system redirection** are selected.</span></span>

> [!IMPORTANT]
> <span data-ttu-id="17e77-446">È molto importante che i passaggi della sequenza di attività siano nell'ordine specificato.</span><span class="sxs-lookup"><span data-stu-id="17e77-446">It is very important that the task sequence steps must be in the provided order.</span></span> <span data-ttu-id="17e77-447">La modifica dell'ordine dei passaggi o la configurazione di passaggi aggiuntivi potrebbero interrompere la distribuzione.</span><span class="sxs-lookup"><span data-stu-id="17e77-447">Modifying the order of steps, or configuring additional steps might break the deployment.</span></span>
>
> <span data-ttu-id="17e77-448">**Impostare e configurare il passaggio dell'applicazione SRS** deve essere l'ultimo passaggio della sequenza di attività, altrimenti la distribuzione potrebbe non riuscire.</span><span class="sxs-lookup"><span data-stu-id="17e77-448">**Set up and configure SRS application** step must be the last step in the task sequence, otherwise the deployment might fail.</span></span>

### <a name="create-deployment-for-the-task-sequence"></a><span data-ttu-id="17e77-449">Creare la distribuzione per la sequenza di attività</span><span class="sxs-lookup"><span data-stu-id="17e77-449">Create deployment for the task sequence</span></span>

1. <span data-ttu-id="17e77-450">Selezionare la sequenza di attività e quindi fare clic su **Distribuisci**.</span><span class="sxs-lookup"><span data-stu-id="17e77-450">Select the task sequence, and then select **Deploy**.</span></span>

2. <span data-ttu-id="17e77-451">Selezionare **Sfoglia** per selezionare la raccolta di destinazione per la distribuzione.</span><span class="sxs-lookup"><span data-stu-id="17e77-451">Select **Browse** to select target collection for deployment.</span></span>

3. <span data-ttu-id="17e77-452">Selezionare **tutti i computer sconosciuti** e quindi fare clic su **OK**.</span><span class="sxs-lookup"><span data-stu-id="17e77-452">Select **All Unknown Computers** and then select **OK**.</span></span>

4. <span data-ttu-id="17e77-453">Selezionare **Avanti**.</span><span class="sxs-lookup"><span data-stu-id="17e77-453">Select **Next**.</span></span>

5. <span data-ttu-id="17e77-454">Selezionare **disponibile** nell'elenco a discesa **scopo** .</span><span class="sxs-lookup"><span data-stu-id="17e77-454">Select **Available** on the **Purpose** drop down list.</span></span>

6. <span data-ttu-id="17e77-455">Selezionare **solo elementi multimediali e PXE** nell'elenco **Rendi disponibile per il seguente** e quindi selezionare **Avanti**.</span><span class="sxs-lookup"><span data-stu-id="17e77-455">Select **Only Media and PXE** in the **Make available to the following** list, and then select **Next**.</span></span>
   > [!WARNING]
   > <span data-ttu-id="17e77-456">È molto importante che **lo scopo** sia impostato su **available**.</span><span class="sxs-lookup"><span data-stu-id="17e77-456">It is very important that **Purpose** is set to **Available**.</span></span> <span data-ttu-id="17e77-457">Verificare che lo **scopo** **non** sia impostato su **obbligatorio**.</span><span class="sxs-lookup"><span data-stu-id="17e77-457">Make sure that the **Purpose** is **NOT** set to **Required**.</span></span> <span data-ttu-id="17e77-458">Assicurarsi inoltre di selezionare **solo elementi multimediali e PXE** nella finestra **Rendi disponibile per le operazioni seguenti**.</span><span class="sxs-lookup"><span data-stu-id="17e77-458">Also make sure that you select **Only Media and PXE** in the **Make available to the following**.</span></span>
   >
   > <span data-ttu-id="17e77-459">L'impostazione di questi valori su qualcos'altro può causare l'avvio di tutti i computer per ottenere l'immagine di distribuzione delle sale di Microsoft teams.</span><span class="sxs-lookup"><span data-stu-id="17e77-459">Setting these values to something else might cause all computers to get the Microsoft Teams Rooms deployment image when booted.</span></span>
7. <span data-ttu-id="17e77-460">Non specificare alcuna pianificazione e quindi selezionare **Avanti**.</span><span class="sxs-lookup"><span data-stu-id="17e77-460">Do not specify any schedule and select **Next**.</span></span>

8. <span data-ttu-id="17e77-461">Non modificare nulla nella sezione **esperienza utente** e selezionare **Avanti**.</span><span class="sxs-lookup"><span data-stu-id="17e77-461">Do not change anything within the **User Experience** section and select **Next**.</span></span>

9. <span data-ttu-id="17e77-462">Non modificare nulla nella sezione **avvisi** e selezionare **Avanti**.</span><span class="sxs-lookup"><span data-stu-id="17e77-462">Do not change anything within the **Alerts** section and select **Next**.</span></span>

10. <span data-ttu-id="17e77-463">Non modificare nulla nella sezione **punti di distribuzione** e selezionare **Avanti**.</span><span class="sxs-lookup"><span data-stu-id="17e77-463">Do not change anything within the **Distribution Points** section and select **Next**.</span></span>

11. <span data-ttu-id="17e77-464">Confermare le impostazioni e quindi fare clic su **Avanti**.</span><span class="sxs-lookup"><span data-stu-id="17e77-464">Confirm the settings and then select **Next**.</span></span>

12. <span data-ttu-id="17e77-465">Selezionare **Chiudi**.</span><span class="sxs-lookup"><span data-stu-id="17e77-465">Select **Close**.</span></span>

<a name="validate-and-troubleshoot-the-solution"></a><span data-ttu-id="17e77-466">Convalidare e risolvere i problemi della soluzione</span><span class="sxs-lookup"><span data-stu-id="17e77-466">Validate and troubleshoot the solution</span></span>
--------------------------------------

<span data-ttu-id="17e77-467">Dopo aver completato le sequenze di attività di Microsoft endpoint Configuration Manager, è necessario eseguire un'esecuzione di test per verificare che la sequenza di attività sia in grado di distribuire e configurare le unità di Microsoft teams rooms.</span><span class="sxs-lookup"><span data-stu-id="17e77-467">After you’ve completed the Microsoft Endpoint Configuration Manager task sequences, you’ll need to perform a test run to validate that the task sequence can deploy and configure Microsoft Teams Rooms units.</span></span>

1.  <span data-ttu-id="17e77-468">Connettere il dispositivo di test alla rete cablata usando una delle schede Ethernet supportate o usando Surface Dock.</span><span class="sxs-lookup"><span data-stu-id="17e77-468">Connect the test device to the wired network by using one of the supported Ethernet adapters or using the Surface dock.</span></span> <span data-ttu-id="17e77-469">Se la funzionalità di avvio PXE non è stata configurata per l'ambiente in uso, è possibile usare l'immagine di avvio dell'unità flash USB [creata in precedenza](https://docs.microsoft.com/configmgr/osd/deploy-use/create-bootable-media) per l'avvio da USB e la connessione a Gestione configurazione.</span><span class="sxs-lookup"><span data-stu-id="17e77-469">If PXE boot functionality hasn’t been configured for your environment, you can use the boot image on the USB flash drive [that you created earlier](https://docs.microsoft.com/configmgr/osd/deploy-use/create-bootable-media) to boot from USB and connect to Configuration Manager.</span></span>

2.  <span data-ttu-id="17e77-470">Accedere al firmware e avviare l'avvio PXE:</span><span class="sxs-lookup"><span data-stu-id="17e77-470">Access the firmware and initiate PXE boot:</span></span>

    1.  <span data-ttu-id="17e77-471">Verificare che il dispositivo Surface sia spento.</span><span class="sxs-lookup"><span data-stu-id="17e77-471">Ensure the Surface device is powered off.</span></span>

    2.  <span data-ttu-id="17e77-472">Premere e tenere premuto il pulsante **volume su** .</span><span class="sxs-lookup"><span data-stu-id="17e77-472">Press and hold the **Volume Up** button.</span></span>

    3.  <span data-ttu-id="17e77-473">Premere e rilasciare il pulsante di **accensione** .</span><span class="sxs-lookup"><span data-stu-id="17e77-473">Press and release the **Power** button.</span></span>

    4.  <span data-ttu-id="17e77-474">Dopo l'avvio del dispositivo, rilasciare il pulsante **volume su** .</span><span class="sxs-lookup"><span data-stu-id="17e77-474">After the device begins to boot, release the **Volume Up** button.</span></span>

    5.  <span data-ttu-id="17e77-475">Selezionare **configurazione di avvio**.</span><span class="sxs-lookup"><span data-stu-id="17e77-475">Select **Boot configuration**.</span></span>

    6.  <span data-ttu-id="17e77-476">Esegui una delle operazioni seguenti:</span><span class="sxs-lookup"><span data-stu-id="17e77-476">Do one of the following:</span></span>

        -   <span data-ttu-id="17e77-477">Selezionare **avvio PXE**e trascinarlo nella parte superiore dell'elenco.</span><span class="sxs-lookup"><span data-stu-id="17e77-477">Select **PXE boot**, and drag it to the top of the list.</span></span> <span data-ttu-id="17e77-478">In alternativa, è possibile scorrere rapidamente verso sinistra sulla scheda di rete per avviare immediatamente il dispositivo.</span><span class="sxs-lookup"><span data-stu-id="17e77-478">Alternatively, you can swipe left on the network adapter to boot to the device immediately.</span></span> <span data-ttu-id="17e77-479">Questo non influirà sull'ordine di avvio.</span><span class="sxs-lookup"><span data-stu-id="17e77-479">This won’t affect the boot order.</span></span>
        -   <span data-ttu-id="17e77-480">Selezionare l'unità flash USB che contiene il supporto di avvio.</span><span class="sxs-lookup"><span data-stu-id="17e77-480">Select the USB flash drive that holds the boot media.</span></span>

3.  <span data-ttu-id="17e77-481">Selezionare **Esci**e quindi **Riavvia ora**.</span><span class="sxs-lookup"><span data-stu-id="17e77-481">Select **Exit**, and then select **Restart Now**.</span></span>

4.  <span data-ttu-id="17e77-482">Quando richiesto, selezionare **invio** per il servizio di avvio di rete.</span><span class="sxs-lookup"><span data-stu-id="17e77-482">When prompted, select **Enter** for network boot service.</span></span>

5.  <span data-ttu-id="17e77-483">Windows PE verrà caricato in memoria e verrà avviata la procedura guidata per la sequenza di attività.</span><span class="sxs-lookup"><span data-stu-id="17e77-483">Windows PE will load into memory, and the Task Sequence Wizard will start.</span></span> <span data-ttu-id="17e77-484">Selezionare **Avanti** per continuare.</span><span class="sxs-lookup"><span data-stu-id="17e77-484">Select **Next** to continue.</span></span>

6.  <span data-ttu-id="17e77-485">Selezionare la sequenza di attività importata in precedenza e quindi fare clic su **Avanti**.</span><span class="sxs-lookup"><span data-stu-id="17e77-485">Select the task sequence that you imported earlier, and then select **Next**.</span></span>

7.  <span data-ttu-id="17e77-486">Dopo aver applicato la configurazione del disco, verrà chiesto di specificare un nome di computer per il dispositivo.</span><span class="sxs-lookup"><span data-stu-id="17e77-486">After the disk configuration is applied, you’ll be prompted to specify a computer name for the device.</span></span> <span data-ttu-id="17e77-487">L'interfaccia utente visualizzerà un nome di computer consigliato in base al numero seriale del dispositivo Surface Pro.</span><span class="sxs-lookup"><span data-stu-id="17e77-487">The user interface will display a recommended computer name based on the serial number of the Surface Pro device.</span></span> <span data-ttu-id="17e77-488">Puoi accettare il nome proposto o specificarne uno nuovo.</span><span class="sxs-lookup"><span data-stu-id="17e77-488">You can either accept the proposed name or specify a new one.</span></span> <span data-ttu-id="17e77-489">Seguire le istruzioni nella schermata Assegnazione nome computer.</span><span class="sxs-lookup"><span data-stu-id="17e77-489">Follow the instructions on the computer name assignment screen.</span></span> <span data-ttu-id="17e77-490">Quando si seleziona **accetta**, viene avviata la distribuzione.</span><span class="sxs-lookup"><span data-stu-id="17e77-490">When you select **Accept**, the deployment begins.</span></span>

8.  <span data-ttu-id="17e77-491">Il resto del processo di distribuzione è automatico e non richiede più input da parte dell'utente.</span><span class="sxs-lookup"><span data-stu-id="17e77-491">The rest of the deployment process is automatic and doesn’t ask for any more user input.</span></span>

9.  <span data-ttu-id="17e77-492">Al termine della configurazione dell'attività di distribuzione, verrà visualizzata la schermata di configurazione seguente che chiede di configurare le impostazioni dell'applicazione Microsoft teams rooms.</span><span class="sxs-lookup"><span data-stu-id="17e77-492">After the deployment task sequence finishes configuring the device, you’ll see the following configuration screen that asks you to configure the Microsoft Teams Rooms application settings.</span></span>

    ![Schermata di configurazione iniziale per l'applicazione Microsoft teams rooms](../media/room-systems-scale-image2.png)

10.  <span data-ttu-id="17e77-494">Collegare Surface Pro alla console Microsoft teams Rooms e configurare le impostazioni dell'applicazione.</span><span class="sxs-lookup"><span data-stu-id="17e77-494">Plug the Surface Pro into the Microsoft Teams Rooms console, and configure the application settings.</span></span>

11.  <span data-ttu-id="17e77-495">Verificare che le funzionalità elencate nella [Guida di Microsoft teams Rooms](https://support.office.com/article/Skype-Room-Systems-version-2-help-e667f40e-5aab-40c1-bd68-611fe0002ba2) stiano lavorando sul dispositivo distribuito.</span><span class="sxs-lookup"><span data-stu-id="17e77-495">Validate that the capabilities listed in [Microsoft Teams Rooms help](https://support.office.com/article/Skype-Room-Systems-version-2-help-e667f40e-5aab-40c1-bd68-611fe0002ba2) are working on the deployed device.</span></span>


<span data-ttu-id="17e77-496">Per risolvere i problemi di un'installazione non riuscita, selezionare il file **Smsts. log** , che registra tutti i passaggi eseguiti in una sequenza di attività di gestione configurazione.</span><span class="sxs-lookup"><span data-stu-id="17e77-496">To troubleshoot a failed installation, check the **SMSTS.log** file, which logs all the steps executed in a Configuration Manager task sequence.</span></span>

<span data-ttu-id="17e77-497">Il file SMSTS. log viene archiviato in uno di un certo numero di percorsi, a seconda della fase del processo di compilazione.</span><span class="sxs-lookup"><span data-stu-id="17e77-497">The SMSTS.log file is stored on one of a number of paths, depending on the stage of the build process.</span></span> <span data-ttu-id="17e77-498">Selezionare la tabella seguente per identificare il percorso del file SMSTS. log.</span><span class="sxs-lookup"><span data-stu-id="17e77-498">Check the following table to identify the path to the SMSTS.log file.</span></span>


| <span data-ttu-id="17e77-499">**Fase di distribuzione**</span><span class="sxs-lookup"><span data-stu-id="17e77-499">**Deployment phase**</span></span>                                                            | <span data-ttu-id="17e77-500">**Percorso log sequenza attività**</span><span class="sxs-lookup"><span data-stu-id="17e77-500">**Task sequence log path**</span></span>                         |
|---------------------------------------------------------------------------------|----------------------------------------------------|
| <span data-ttu-id="17e77-501">WinPE, prima del formato HDD</span><span class="sxs-lookup"><span data-stu-id="17e77-501">WinPE, before HDD format</span></span>                                                        | <span data-ttu-id="17e77-502">X:\\Windows\\temp\\smstslog\\Smsts. log</span><span class="sxs-lookup"><span data-stu-id="17e77-502">X:\\Windows\\Temp\\smstslog\\smsts.log</span></span>             |
| <span data-ttu-id="17e77-503">WinPE, dopo il formato HDD</span><span class="sxs-lookup"><span data-stu-id="17e77-503">WinPE, after HDD format</span></span>                                                         | <span data-ttu-id="17e77-504">C:\\_SMSTaskSequence\\log\\Smstslog\\Smsts. log</span><span class="sxs-lookup"><span data-stu-id="17e77-504">C:\\_SMSTaskSequence\\Logs\\Smstslog\\smsts.log</span></span>    |
| <span data-ttu-id="17e77-505">Sistema operativo distribuito prima dell'installazione dell'agente Configuration Manager</span><span class="sxs-lookup"><span data-stu-id="17e77-505">Operating system deployed, before the Configuration Manager agent was installed</span></span> | <span data-ttu-id="17e77-506">c:\\_SMSTaskSequence\\log\\Smstslog\\Smsts. log</span><span class="sxs-lookup"><span data-stu-id="17e77-506">c:\\_SMSTaskSequence\\Logs\\Smstslog\\smsts.log</span></span>    |
| <span data-ttu-id="17e77-507">Sistema operativo e agente Configuration Manager distribuito</span><span class="sxs-lookup"><span data-stu-id="17e77-507">Operating system and the Configuration Manager agent deployed</span></span>                   | <span data-ttu-id="17e77-508">% WINDIR%\\system32\\CCM\\logs\\Smstslog\\Smsts. log</span><span class="sxs-lookup"><span data-stu-id="17e77-508">%windir%\\System32\\ccm\\logs\\Smstslog\\smsts.log</span></span> |
| <span data-ttu-id="17e77-509">Esecuzione della sequenza di attività completata</span><span class="sxs-lookup"><span data-stu-id="17e77-509">Task sequence execution complete</span></span>                                                | <span data-ttu-id="17e77-510">% WINDIR%\\system32\\CCM\\logs\\Smsts. log</span><span class="sxs-lookup"><span data-stu-id="17e77-510">%windir%\\System32\\ccm\\logs\\smsts.log</span></span>           |

> [!TIP]
> <span data-ttu-id="17e77-511">È possibile selezionare **F8** in qualsiasi momento durante la sequenza di attività per aprire una console dei comandi e quindi accedere al file Smsts. log.</span><span class="sxs-lookup"><span data-stu-id="17e77-511">You can select **F8** at any time during the task sequence to open a command console, and then get access to the SMSTS.log file.</span></span>

<span data-ttu-id="17e77-512">Per risolvere i problemi di avvio PXE, controllare i due file di log nel server Configuration Manager specifici delle azioni PXE:</span><span class="sxs-lookup"><span data-stu-id="17e77-512">To troubleshoot PXE boot issues, check the two log files on the Configuration Manager server that are specific to PXE actions:</span></span>

-   <span data-ttu-id="17e77-513">**Pxecontrol. log**, disponibile nella directory log di installazione di Configuration Manager</span><span class="sxs-lookup"><span data-stu-id="17e77-513">**Pxecontrol.log**, located in the Configuration Manager installation logs directory</span></span>

-   <span data-ttu-id="17e77-514">**SMSPXE. log**, situato nella directory dei registri di Configuration Manager Management Point (MP)</span><span class="sxs-lookup"><span data-stu-id="17e77-514">**Smspxe.log**, located in Configuration Manager Management Point (MP) logs directory</span></span>

<span data-ttu-id="17e77-515">Per un elenco completo dei file di log che è possibile usare per risolvere ulteriormente la procedura di installazione di Configuration Manager, vedere il [riferimento al file di log](https://docs.microsoft.com/configmgr/core/plan-design/hierarchy/log-files)di Microsoft endpoint Configuration Manager.</span><span class="sxs-lookup"><span data-stu-id="17e77-515">For a complete list of the log files that you can use to further troubleshoot your Configuration Manager installation, see the Microsoft Endpoint Configuration Manager [Log file reference](https://docs.microsoft.com/configmgr/core/plan-design/hierarchy/log-files).</span></span>
