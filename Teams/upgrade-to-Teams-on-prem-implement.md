...
Se sono necessarie altre informazioni sulle opzioni, verificare di aver già letto Scegliere il percorso di aggiornamento da [Skype for Business a Teams](upgrade-and-coexistence-of-skypeforbusiness-and-teams.md).
...
![Diagramma che mostra l'aggiornamento delle funzionalità di selezione senza l'uso Teams.](media/teams-upgrade-1.png)
...
Se alcuni utenti dell'organizzazione usano attivamente Teams in modalità Isole, è probabile che non si voglia rimuovere la funzionalità dagli utenti esistenti. Pertanto, prima di modificare i criteri a livello di tenant è necessario un passaggio aggiuntivo. La soluzione consiste nel "nonno" di questi utenti attivi Teams in modalità Isole, prima di impostare i criteri a livello di tenant su SfbWithTeamsCollab.  Dopo aver eseguito questa operazione, è possibile procedere con la distribuzione come sopra, tuttavia, si avranno due gruppi di utenti che si spostano in TeamsOnly: gli utenti attivi in Teams saranno in modalità Isole e gli utenti rimanenti saranno in modalità SfbWithTeamsCollab. È possibile spostare progressivamente questi utenti in modalità TeamsOnly.

1. Trovare gli utenti attivi in Teams nel modo seguente:

   1. Nel riquadro interfaccia di amministrazione di Microsoft 365 spostamento a sinistra passare a Report e quindi a Uso. 
   2. Nell'elenco a discesa "Seleziona un report" scegliere Microsoft Teams e quindi Attività utente. In questo modo verrà mostrata una tabella esportabile di utenti che sono stati attivi in Teams. 
   3. Fare clic su Esporta, Excel e filtrare per visualizzare solo gli utenti attivi in Teams.

2. Per ogni utente Teams utente trovato nel passaggio 1, assegnargli la modalità Isole in una sessione remota di PowerShell. In questo modo è possibile andare al passaggio successivo e assicurarsi di non modificare l'esperienza utente.  
