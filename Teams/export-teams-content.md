---
title: Esportare contenuto con le API di esportazione Microsoft Teams
author: SerdarSoysal
ms.author: serdars
manager: serdars
ms.topic: reference
...
description: In questo articolo vengono fornite informazioni su come esportare Teams contenuto usando le API di esportazione Microsoft Teams.
...
---
# <a name="export-content-with-the-microsoft-teams-export-apis"></a>Esportare contenuto con le API di esportazione Microsoft Teams

Teams Esporta API consentono di esportare 1:1, chat di gruppo, chat di riunione e messaggi di canale da Microsoft Teams. Se l'organizzazione deve esportare i messaggi di Microsoft Teams, è possibile estrarli usando Teams le API di esportazione. *Messaggio di chat* rappresenta un singolo messaggio di chat all'interno di un [canale](/graph/api/resources/channel) o [di una chat](/graph/api/resources/chat). Il messaggio di chat può essere un messaggio di chat radice o parte di un thread di risposta definito dalla proprietà **replyToId** nel messaggio di chat.

Ecco alcuni esempi su come usare queste API di esportazione:

- **Esempio 1**: se è stato abilitato Microsoft Teams nell'organizzazione e si vogliono esportare tutti i messaggi Microsoft Teams a livello di programmazione passando l'intervallo di date per un determinato utente o team.
...
## <a name="what-is-supported-by-the-teams-export-apis"></a>Cosa è supportato dalle API di esportazione Teams?

- **Esportazione in blocco di Teams messaggio:** Teams le API di esportazione supportano fino a 200 RPS per app Per tenant e 600 RPS per un'applicazione, con questi limiti dovrebbe essere possibile esportare in blocco i messaggi di Teams.
...
- **Messaggi eliminati dall'utente:** È possibile accedere ai messaggi eliminati dagli utenti dal client Teams usando le API di esportazione fino a 21 giorni dal momento dell'eliminazione.
...
- **Proprietà dei messaggi di chat:** Fai riferimento all'elenco completo delle proprietà che Teams supporto per l'esportazione delle API [qui](/graph/api/resources/chatmessage#properties).

> [!NOTE]
> Le API di esportazione non supportano *le reazioni*.

## <a name="how-to-access-teams-export-apis"></a>Come accedere alle API di esportazione Teams
...
## <a name="prerequisites-to-access-teams-export-apis"></a>Prerequisiti per accedere alle API di esportazione Teams

- Microsoft Teams API di Microsoft Graph che accedono a dati sensibili sono considerate API protette. Le API di esportazione richiedono una convalida aggiuntiva, oltre alle autorizzazioni e al consenso, prima di poterle usare. Per richiedere l'accesso a queste API protette, completa il [modulo di richiesta](https://aka.ms/teamsgraph/requestaccess).
