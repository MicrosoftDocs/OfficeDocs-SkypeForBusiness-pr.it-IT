...
|AllowIPVideo |Determina se il video è abilitato nelle riunioni o nelle chiamate di un utente. | Impostare su false per impedire all'utente di condividere il video. |
| AllowAnonymousUsersToDialOut | Determina se gli utenti anonimi possono effettuare la chiamata a un numero PSTN. | Impostare su false per impedire la chiamata degli utenti anonimi. |
...
| AllowOutlookAddIn |Determina se un utente può pianificare le riunioni di Teams nel client desktop di Outlook. | Impostare su false per impedire a un utente di pianificare le riunioni di Teams nel client desktop di Outlook. |
...
| AllowExternalParticipantGiveRequestControl | Determina se i partecipanti esterni possono richiedere o dare il controllo della condivisione dello schermo. | Impostare su false per impedire l'assegnazione di un utente esterno, richiedendo il controllo in una riunione. |
| AllowPowerPointSharing |Determina se la condivisione di PowerPoint è consentita nelle riunioni di un utente. |Impostare su false per impedire a un utente di condividere i file di PowerPoint in una riunione. |
...
| AllowSharedNotes | Determina se gli utenti possono prendere note condivise. | Impostare su false per impedire agli utenti di accettare note condivise. |
...
1. Avviare una sessione di Windows PowerShell come amministratore.
1. Connettersi al connettore Skype online.
...
1. Visualizzare un elenco delle opzioni dei criteri di riunione.

      ```powershell
      Get-CsTeamsMeetingPolicy
      ```

1. Cercare l'opzione criteri predefiniti in cui sono disabilitati tutti i criteri riunione. Ha un aspetto simile a questo.
...
1. Applicare l'opzione dei criteri predefiniti di AllOff a tutti gli utenti che useranno teams in un ambiente virtualizzato.
...
1. Eseguire il comando seguente per installare il file MSI nella VM VDI o per completare l'aggiornamento.
...
1. Eseguire il comando seguente per disinstallare il file MSI dalla VM VDI o prepararsi per l'aggiornamento.
