...
Virtual Desktop Infrastructure (VDI) è una tecnologia di virtualizzazione che ospita un sistema operativo desktop e applicazioni in un server centralizzato in un data center. Questo consente un'esperienza desktop completa e personalizzata per gli utenti con una fonte centralizzata completamente protetta e conforme.

Teams in un ambiente virtualizzato supporta la chat e la collaborazione. Inoltre, con le piattaforme Azure Virtual Desktop, Citrix e VMware, sono supportate anche le funzionalità di chiamata e riunione.

Teams supporta anche più configurazioni in ambienti virtuali. Sono incluse le modalità VDI, dedicate, condivise, persistenti e non persistenti. Le funzionalità sono in continuo sviluppo e vengono aggiunte regolarmente e le funzionalità si espanderanno nel tempo.
...
- **Desktop virtuale**: stack di macchine virtuali che viene eseguito Teams
- **Thin client**: il dispositivo con cui l'utente si interfaccia fisicamente
...
|----|---|
|![Il logo che rappresenta Microsoft.](media/microsoft-logo.png)| <a href="/azure/virtual-desktop/teams-on-wvd" target="_blank">Desktop virtuale di Azure</a>, <a href="/windows-365/enterprise/teams-on-cloud-pc" target="_blank">Windows 365</a> |
...
Azure Virtual Desktop offre l'ottimizzazione AV Teams in VDI. Per altre informazioni sui requisiti e l'installazione, vedere Usare Teams desktop virtuale di Azure.To learn more on requirements and installation, see [Use Teams on Azure Virtual Desktop](/azure/virtual-desktop/teams-on-wvd).

### <a name="windows-365"></a>Windows 365

Windows 365 usa l'ottimizzazione AV fornita da Azure Virtual Desktop per garantire un'esperienza Teams ottimale dai PC cloud. Per altre informazioni sui requisiti e l'installazione, vedere Usare Teams [su Cloud PC](/windows-365/enterprise/teams-on-cloud-pc).
...
Per i requisiti più recenti per server e client, vedere l'articolo [Ottimizzazione](https://docs.citrix.com/en-us/citrix-virtual-apps-desktops/multimedia/opt-ms-teams.html) per Microsoft Teams nel sito Web Di Citrix.

### <a name="vmware-horizon-workspace-and-desktop-requirements"></a>Requisiti di VMware Horizon Workspace e desktop

VMware Horizon è una piattaforma moderna per la distribuzione sicura di desktop virtuali e app nel cloud ibrido. Per offrire un'esperienza utente ottimale, VMware Horizon offre l'ottimizzazione dei supporti multimediali Teams. Questa ottimizzazione migliora la produttività complessiva di desktop e app virtuali e migliora l'esperienza utente durante le chiamate e le riunioni con Teams.
...
Per una configurazione persistente dedicata, l'installazione per computer e per utente funzionerà. Tuttavia, per una configurazione non persistente, Teams un'installazione per computer per lavorare in modo efficiente. Vedere la [sezione Configurazione non persistente](#non-persistent-setup) .
...
Per la maggior parte delle distribuzioni VDI, è consigliabile distribuire Teams con l'installazione per computer. Per eseguire l'aggiornamento alla versione Teams versione più recente, iniziare con la procedura di disinstallazione seguita dalla distribuzione Teams versione più recente.

Per Teams'ottimizzazione AV in ambienti VDI per il corretto funzionamento, il dispositivo thin client deve avere accesso a Internet. Se l'accesso a Internet non è disponibile nel dispositivo thin client, l'avvio dell'ottimizzazione non riesce. Questo significa che l'utente è in uno stato multimediale non ottimizzato.
...
|---------|---------|---------|
|vCPU   |    2 core     |  4, 6 o 8 core<br>È importante comprendere la configurazione NUMA (Non Uniform Memory Access) sottostante e configurare di conseguenza le macchine virtuali.     |
|RAM     |   4 GB      | Da 512 MB a 1 GB per utente        |
...
Per una configurazione non persistente, Teams'app desktop deve essere installata per computer nell'immagine dorata. In questo modo si garantisce un avvio efficiente dell'app Teams durante una sessione utente. Per altre informazioni, vedere la sezione [Installare o aggiornare l Teams app desktop in VDI](#install-or-update-the-teams-desktop-app-on-vdi).

L Teams in una configurazione non persistente richiede anche un gestore della cache dei profili per una sincronizzazione efficiente Teams dati di runtime. Una sincronizzazione efficiente dei dati assicura che le informazioni appropriate specifiche dell'utente, ad esempio i dati, il profilo o le impostazioni di un utente, siano memorizzate nella cache durante la sessione dell'utente. Assicurarsi che i dati in queste due cartelle siano sincronizzati:<br>
...
Escludere quanto segue dalla Teams di memorizzazione nella cache, `%AppData%/Microsoft/Teams`. L'esclusione di questi elementi consente di ridurre le dimensioni della cache degli utenti per ottimizzare ulteriormente la configurazione non persistente.
...
Teams tramite Microsoft 365 Apps for enterprise viene installato per utente. Per altre informazioni, vedere la sezione [Installare o aggiornare l Teams app desktop in VDI](#install-or-update-the-teams-desktop-app-on-vdi).
...
Microsoft 365 Apps for enterprise non supporta le installazioni per computer di Teams. Per usare l'installazione per computer, è necessario escludere Teams da Microsoft 365 Apps for enterprise. Vedere le [sezioni Distribuire l'app desktop Teams](#deploy-the-teams-desktop-app-to-the-vm) nella macchina virtuale e Come escludere Teams [distribuzione tramite](#how-to-exclude-teams-deployment-through-microsoft-365-apps-for-enterprise) Microsoft 365 Apps for enterprise distribuzione.
...
1. Scaricare il Teams MSI che corrisponde al sistema operativo VDI VM usando uno dei collegamenti seguenti:
    ...
    > [!NOTE]
    > Per i cloud per enti pubblici, vedere Installare Teams in blocco [usando Windows Installer (MSI)](msi-deployment.md) per i collegamenti di download ai file MSI.
...
2. Installare il file MSI nella macchina virtuale VDI eseguendo uno dei comandi seguenti:
    ...
    - Installazione per utente (impostazione predefinita)
        ...
        Questo processo è l'installazione predefinita, che Teams nella cartella `%AppData%` utente. A questo punto, la configurazione dell'immagine dorata è completata.
    ...
    - Installazione per computer
        ...
        Questo processo Teams `%ProgramFiles(x86)%` nella cartella in un sistema operativo a 64 bit `%ProgramFiles%` e nella cartella in un sistema operativo a 32 bit. A questo punto, la configurazione dell'immagine dorata è completata.

        > [!IMPORTANT]
        >  L'Teams per computer è necessario per le configurazioni non persistenti.

        Quando viene avviata la sessione di accesso interattiva successiva, Teams e chiede le credenziali.

        > [!NOTE]
        > In questi esempi viene anche utilizzato il `ALLUSERS=1` parametro . Quando si imposta questo parametro, **Teams Machine-Wide installer** viene **visualizzato in Programmi** e funzionalità nel Pannello di  controllo e nelle funzionalità di **&** app in **Windows Impostazioni** per tutti gli utenti del computer. Tutti gli utenti possono quindi disinstallare Teams se hanno credenziali di amministratore.
        >
        > È importante comprendere la differenza `ALLUSERS=1` tra e `ALLUSER=1`. Il `ALLUSERS=1` parametro può essere usato in ambienti non VDI e VDI, `ALLUSER=1` mentre il parametro viene usato solo negli ambienti VDI per specificare un'installazione per computer.

3. Disinstallare il file MSI dalla macchina virtuale VDI. Esistono due modi per disinstallare Teams.

    - **Script di PowerShell**: è possibile usare lo script di [PowerShell](scripts/powershell-script-deployment-cleanup.md) Teams pulizia della distribuzione per disinstallare Teams e rimuovere la cartella Teams per un utente. Eseguire lo script per ogni profilo utente in cui Teams è stato installato nel computer.
    - **Riga di comando**: eseguire il comando seguente.
      ...
      Questo processo disinstalla Teams dalla `%ProgramFiles(x86)%` cartella o `%ProgramFiles%` dalla cartella, a seconda dell'ambiente del sistema operativo.
...
Per altre informazioni su come preparare la rete per la Teams, vedere Preparare la rete [dell'organizzazione](prepare-network.md) per Teams.
...
Se si esegue la migrazione da Skype for Business in VDI a Teams in VDI, oltre alle differenze tra le due applicazioni, esistono alcune differenze quando viene implementato anche VDI. Alcune funzionalità attualmente non supportate in Teams VDI Skype for Business VDI sono le seguenti:
...
### <a name="teams-on-chrome-browser-versus-teams-desktop-app-for-vdi"></a>Teams browser Chrome rispetto all Teams app desktop per VDI

Teams browser Chrome non fornisce una sostituzione dell'app desktop Teams per VDI con l'ottimizzazione AV. L'esperienza di chat e collaborazione funziona come previsto. Quando sono necessari elementi multimediali, ci sono alcune esperienze che potrebbero non soddisfare le aspettative degli utenti nel browser Chrome:

- L'esperienza di streaming audio e video potrebbe non essere ottimale. Gli utenti potrebbero subire ritardi o ridurre la qualità.
...
È possibile impostare criteri usando l'interfaccia Teams o PowerShell. La propagazione delle modifiche ai criteri può essere fino a poche ore. Se le modifiche per un determinato account non sono immediatamente disponibili, riprovare tra qualche ora.
...
#### <a name="assign-policies-using-the-teams-admin-center"></a>Assegnare criteri usando l'interfaccia Teams di amministrazione

Per assegnare i **criteri di chiamata DisallowCalling** e **i criteri della riunione AllOff** a un utente:

1. Nel riquadro di spostamento sinistro dell'Teams di amministrazione passare a **Utenti**.
...
1. Nel riquadro di spostamento sinistro dell Teams di amministrazione passare a **Utenti e quindi** cercare gli utenti o filtrare la visualizzazione per visualizzare gli utenti desiderati.
...
1. Nel riquadro di spostamento sinistro dell'Teams di amministrazione passare al criterio da assegnare. Ad esempio:
    - Passare ai **criteri** **VoiceCalling** >  e quindi fare clic **su DisallowCalling**.
    - Passare a **Criteri di MeetingsMeeting** e quindi fare clic su **AllOff.Go** to **MeetingsMeeting** >  policies, and then click AllOff.
...
L'esempio seguente mostra come usare [Grant-CsTeamsCallingPolicy](/powershell/module/skype/grant-csteamscallingpolicy) per assegnare `DisallowCalling` i criteri di chiamata a un utente.
...
L'esempio seguente mostra come usare [Grant-CsTeamsMeetingPolicy](/powershell/module/skype/grant-csteamsmeetingpolicy) `AllOff` per assegnare i criteri riunione a un utente.
...
Se si ha un'implementazione esistente di Teams in VDI con chat e collaborazione in cui sono stati impostati criteri a livello di utente per disattivare le funzionalità di chiamata e riunione e si esegue la migrazione a Teams con l'ottimizzazione AV, è necessario impostare criteri per attivare la funzionalità di chiamata e riunione per gli utenti di Teams in VDI.
...
1. Nel riquadro di spostamento sinistro dell'Teams di amministrazione passare al criterio da assegnare. Ad esempio:
    - Passare ai **criteri** **VoiceCalling** >  e quindi fare clic **su AllowCalling**.
    - Passare a **Criteri di** **MeetingsMeeting** >  e quindi fare clic su **AllOn**.
2. Scegliere **Gestisci utenti**.
3. Nel riquadro **Gestisci utenti** cercare l'utente per nome visualizzato o in base al nome utente, selezionare il nome e poi fare clic su **Aggiungi**. Ripetere questa operazione per ogni utente da aggiungere.
4. Dopo aver aggiunto gli utenti, fare clic su **Salva**.
...
L'esempio seguente mostra come usare [Grant-CsTeamsCallingPolicy](/powershell/module/skype/grant-csteamscallingpolicy) per assegnare `AllowCalling` i criteri di chiamata a un utente.
...
L'esempio seguente mostra come usare [Grant-CsTeamsMeetingPolicy](/powershell/module/skype/grant-csteamsmeetingpolicy) `AllOn` per assegnare i criteri riunione a un utente.
...
Quando gli utenti si connettono da un endpoint non supportato, gli utenti sono in modalità fallback, in cui av non è ottimizzato. È possibile disabilitare o abilitare la modalità di fallback impostando uno dei valori del Registro di sistema `DWORD` seguenti:
...
Quando gli utenti il cui criterio VDI `DisableCallsAndMeetings` è `$true` impostato per l'Teams in VDI, non possono:
...
Quando gli utenti il cui criterio VDI `DisableAudioVideoInCallsAndMeetings` è `$true` impostato per l'accesso a Teams in VDI, possono:
...
#### <a name="teams-crashes-or-the-teams-sign-in-screen-is-blank"></a>Teams si arresta in modo anomalo o la Teams di accesso è vuota

Si tratta di un problema noto con le versioni 1906 e 1909 di Citrix VDA. Per risolvere il problema, aggiungere il valore del `DWORD` Registro di sistema seguente e impostarlo su `204` (esadecimale).
...
Riavviare quindi VDA. Per altre informazioni, vedere questo articolo del supporto tecnico di Citrix, [Risoluzione dei problemi di ottimizzazione HDX per Microsoft Teams](https://support.citrix.com/article/CTX253754).
