...
L'Teams in un ambiente virtualizzato potrebbe essere leggermente diverso dall'Teams in un ambiente non virtualizzato. Ad esempio, alcune funzionalità avanzate potrebbero non essere disponibili in un ambiente virtualizzato e la risoluzione video potrebbe essere diversa.
...
- **Teams app desktop: l'app** Teams client desktop
...
Azure Virtual Desktop offre l'ottimizzazione AV Teams in VDI. Per altre informazioni sui requisiti e l'installazione, vedere Usare Teams [desktop virtuale di Azure](/azure/virtual-desktop/teams-on-wvd).

### <a name="windows-365"></a>Windows 365

Windows 365 usa l'ottimizzazione AV fornita da Desktop virtuale di Azure per garantire esperienze Teams ottimali dai PC cloud. Per altre informazioni sui requisiti e l'installazione, vedere Usare [Teams su Cloud PC](/windows-365/enterprise/teams-on-cloud-pc).
...
Per i requisiti più recenti per server e client, vedere l'articolo [Ottimizzazione](https://docs.citrix.com/en-us/citrix-virtual-apps-desktops/multimedia/opt-ms-teams.html) per Microsoft Teams nel sito Web Citrix.
...
Escludere quanto segue dalla cartella Teams di memorizzazione nella cache, `%AppData%/Microsoft/Teams`. L'esclusione di questi elementi consente di ridurre le dimensioni della cache degli utenti per ottimizzare ulteriormente la configurazione non persistente.
...
#### <a name="new-deployments-of-teams-through-microsoft-365-apps-for-enterprise"></a>Nuove distribuzioni di Teams a Microsoft 365 Apps for enterprise
...
Microsoft 365 Apps for enterprise non supporta le installazioni per computer di Teams. Per usare l'installazione per computer, è necessario escludere Teams da Microsoft 365 Apps for enterprise. Vedere le [sezioni Distribuire l Teams app desktop](#deploy-the-teams-desktop-app-to-the-vm) nella macchina virtuale e Come escludere Teams [distribuzione tramite](#how-to-exclude-teams-deployment-through-microsoft-365-apps-for-enterprise) Microsoft 365 Apps for enterprise distribuzione.
...
2. Installare il file MSI nella macchina virtuale VDI eseguendo uno dei comandi seguenti:
    ...
    - Installazione per computer
        ...
        > [!IMPORTANT]
        >  L'Teams per ogni computer è necessario per le configurazioni non persistenti.
...
3. Disinstallare il file MSI dalla macchina virtuale VDI. Esistono due modi per disinstallare i Teams.

    - **Script di PowerShell**: è possibile usare [](scripts/powershell-script-deployment-cleanup.md) lo script di PowerShell Teams pulizia della distribuzione per disinstallare Teams e rimuovere la cartella Teams per un utente. Eseguire lo script per ogni profilo utente in cui Teams è stato installato nel computer.
    - **Riga di comando**: eseguire il comando seguente.
      ...
      Questo processo disinstalla Teams dalla cartella `%ProgramFiles(x86)%` o `%ProgramFiles%` dalla cartella, a seconda dell'ambiente del sistema operativo.
...
- **Dipendenze**: includono le dipendenze dall'infrastruttura, dal carico di lavoro e da altre considerazioni sull'ambiente all'esterno dell'app desktop Teams desktop.
...
### <a name="teams-on-chrome-browser-versus-teams-desktop-app-for-vdi"></a>Teams browser Chrome rispetto all'app desktop Teams per VDI
...
È possibile impostare i criteri usando l'interfaccia Teams o PowerShell. La propagazione delle modifiche ai criteri può essere fino a poche ore. Se le modifiche per un determinato account non sono immediatamente disponibili, riprovare tra qualche ora.
...
Quando gli utenti il cui criterio VDI `DisableCallsAndMeetings` è impostato `$true` per accedere Teams in VDI, non possono:
...
- Negli ambienti Citrix, se l'utente si disconnette dalla macchina virtuale mentre Teams è in esecuzione, gli aggiornamenti Teams possono comportare che l'utente sia in uno stato non ottimizzato per av quando si riconnette. È consigliabile che gli utenti Teams prima di disconnettersi da Citrix Virtual Machine per evitare questo scenario.
