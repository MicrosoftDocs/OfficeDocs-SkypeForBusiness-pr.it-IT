...
L Teams in un ambiente virtualizzato potrebbe essere leggermente diverso dall'uso di Teams in un ambiente non virtualizzato. Ad esempio, alcune funzionalità avanzate potrebbero non essere disponibili in un ambiente virtualizzato e la risoluzione video potrebbe essere diversa.
...
Azure Virtual Desktop offre l'ottimizzazione AV Teams in VDI. Per altre informazioni sui requisiti e l'installazione, vedere [Usare Teams desktop virtuale di Azure.To](/azure/virtual-desktop/teams-on-wvd) learn more on requirements and installation, see Use Teams on Azure Virtual Desktop.

### <a name="windows-365"></a>Windows 365

Windows 365 usa l'ottimizzazione AV fornita da Azure Virtual Desktop per garantire esperienze Teams ottimali dai PC cloud. Per altre informazioni sui requisiti e l'installazione, vedere Usare [Teams su Cloud PC](/windows-365/enterprise/teams-on-cloud-pc).
...
Per ottenere i requisiti e le istruzioni più recenti su come configurare l'ottimizzazione dei supporti Teams, vedere questo sito Web [di VMware](https://docs.vmware.com/en/VMware-Horizon/2006/horizon-remote-desktop-features/GUID-F68FA7BB-B08F-4EFF-9BB1-1F9FC71F8214.html).
...
Con l'installazione per computer, gli aggiornamenti automatici vengono disabilitati. Questo significa che per aggiornare l'app Teams, è necessario disinstallare la versione corrente per eseguire l'aggiornamento a una versione più recente. Con l'installazione per utente, gli aggiornamenti automatici sono abilitati. Per la maggior parte delle distribuzioni VDI, è consigliabile distribuire Teams con l'installazione per computer.
...
Per una configurazione non persistente, Teams'app desktop deve essere installata per computer nell'immagine dorata. Per altre informazioni, vedere la sezione Installare [o aggiornare Teams'app desktop in VDI](#install-or-update-the-teams-desktop-app-on-vdi). In questo modo si garantisce un avvio efficiente dell Teams app durante una sessione utente.
...
3. Disinstallare il file MSI dalla macchina virtuale VDI. Esistono due modi per disinstallare Teams.
    ...
    - Riga di comando: eseguire il comando seguente.
      ...
      Questo processo Teams dalla cartella Programmi (x86) o Programmi, a seconda dell'ambiente del sistema operativo.
...
## <a name="migrate-teams-on-vdi-with-chat-and-collaboration-to-optimize-teams-with-calling-and-meetings"></a>Eseguire la Teams in VDI con chat e collaborazione per ottimizzare Teams con chiamate e riunioni
...
L'esempio seguente mostra come usare [Grant-CsTeamsMeetingPolicy per assegnare](/powershell/module/skype/grant-csteamsmeetingpolicy) i criteri riunione AllOn a un utente.
...
Teams criteri VDI sono disponibili nel modulo Microsoft Teams. Questi criteri sono attivi e applicati in ambienti VDI non ottimizzati.
