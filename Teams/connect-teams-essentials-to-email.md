...
Dopo la connessione, è possibile visualizzare le risposte alle riunioni pianificate e gli inviti a collaborare nella cassetta postale e Microsoft Teams. È anche possibile visualizzare e interagire con le riunioni in arrivo dal calendario usando Teams software per le riunioni di terze parti come Google Workspace.
...
2. Nel menu di spostamento sinistro passare a Posta **elettronica & collaborazionePolizie** >  & **regoleTrovare** >  >  **criteriAnti-spam** nella sezione Criteri
...
È possibile usufruire di tutto Teams Essentials (AAD) usando un approccio ibrido per configurare la connessione tra Microsoft Teams e Exchange Online con Exchange locale.

Per far funzionare l'accesso al calendario per le cassette postali locali, seguire le istruzioni fornite inConfigurazione dell'accesso al calendario di Teams per le cassette postali locali di [Exchange - Microsoft Tech Community](https://techcommunity.microsoft.com/t5/exchange-team-blog/configuring-teams-calendar-access-for-exchange-on-premises/ba-p/1484009)
...
La sezione seguente illustra come connettersi Microsoft Teams un sistema di posta elettronica esistente con il calendario, ad esempio Google Workspace. Questa connessione verrà eseguita lasciando intatto il sistema di posta elettronica corrente, inoltrando tutti i messaggi di posta elettronica a Exchange Online, filtrando tutto tranne i messaggi di posta elettronica del tipo di calendario. In questo modo, i messaggi di posta elettronica del calendario vengono visualizzati automaticamente nel calendario Teams accettati come messaggi di posta elettronica provvisori e non di tipo calendario vengono eliminati.

Tutti i messaggi generati in Microsoft 365 vengono inoltrati a Google Workspace in modo che gli utenti Teams promemoria e notifiche. Le identità degli utenti, come la posta elettronica principale dell'utente, possono essere duplicate. Single Sign-On è anche possibile, ma non obbligatorio. Gli utenti devono essere in grado di partecipare Teams riunioni dal calendario di terze parti o dal calendario Teams calendario. Altre Teams funzioni funzionano come previsto.
...
4. Quando richiesto, selezionare **Non consentire Microsoft 365 configurare IL DNS**.
...
8. Accedere di nuovo al interfaccia di amministrazione di Microsoft 365 per <https://admin.microsoft.com/AdminPortal/> abilitare DKIM

9. Nel riquadro di spostamento a sinistra selezionare **SetupDomains**  > 
...
2. Passare a **UtentiAttivi** >  utenti e selezionare **Aggiungi utente**
...
***Configurare il recapito doppio della posta elettronica Microsoft 365 e rimuovere gli allegati:***
...
2. Aggiungere un percorso per Office 365
    ...
    - Passare alla Console di amministrazione di Google all'indirizzo <https://admin.google.com>)
...
3. Configurare la route per Office 365
    ...
    - Passare a **AppGoogle** >  **WorkspaceGmailRouting** >  > 

    - Nella scheda **Routing** selezionare **Configura**

    - Immettere **Nome per** la regola. (Ad esempio, Gmail per Office 365)

    - Per **l'effetto dei messaggi di** posta elettronica, selezionare **sia Ricezione in ingresso**  **che Ricezione interna**
...
2. Passare a **AccountDomainsManage** >  >  **Domains**
...
5. Passare a **AppsGoogle** >  **WorkspaceGmailCompliance** >  > 
...
4. Nel menu **Exchange** di spostamento sinistro dell'interfaccia di amministrazione selezionare **FlowConnectors** >  di posta, aprire il menu di riversamento (...) e selezionare Aggiungi un connettore
...
9. Nella sezione Routing immettere lo Smart Mail Host appropriato, ad esempio aspmx.l.google.com, **+** selezionare e quindi continuare **.**
...
1. Usare il **Amministrazione Microsoft 365 Center** per aggiornare ogni cassetta postale oppure è possibile usare uno script **di PowerShell**, ad esempio:
    ...
    **Risoluzione dei Connessione-ExchangeOnline:**

    Si verifica un errore durante l'esecuzione di Connessione-ExchangeOnline? Questo potrebbe essere il risultato della regola di inoltro automatico della posta elettronica dell'organizzazione. Per impostazione predefinita, l'inoltro automatico è disabilitato. Per connettersi Teams Essentials a Google Workspace, la regola deve essere abilitata.  
...
1. Se si configura questa impostazione, gli inviti al calendario verranno accettati automaticamente in modo che siano visualizzati nel calendario Teams senza richiedere agli utenti di interagire con l'invito Outlook Web App.
...
1. Seguire le istruzioni in [Abilitare o disabilitare Outlook sul web per una cassetta postale in](/exchange/recipients-in-exchange-online/manage-user-mailboxes/enable-or-disable-outlook-web-app) Exchange Online disabilitare Outlook sul web per le cassette postali.

2. È possibile disabilitare Outlook sul web **l'interfaccia Exchange o** **PowerShell**. È possibile usare l'esempio di PowerShell seguente per disabilitare Outlook sul web per tutte le cassette postali:
...
1. È possibile configurare questa regola **nell'Exchange di amministrazione o** **in PowerShell**. È possibile usare l'esempio **di PowerShell** seguente per creare la regola:
...
**2. Configurare il tenant Microsoft 365 utenti**

*Configurare Teams AAD utenti*

1. Seguire le istruzioni disponibili in [Aggiungere utenti e assegnare licenze](/microsoft-365/admin/add-users/add-users?view=o365-worldwide&preserve-view=true) per aggiungere più utenti
...
*Aggiungere un dominio a Amministrazione Microsoft 365 Center (facoltativo)*

1. Nella barra di spostamento seleziona Impostazioni > dominio, quindi seleziona Aggiungi dominio
