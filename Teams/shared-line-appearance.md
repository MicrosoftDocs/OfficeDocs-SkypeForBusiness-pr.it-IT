...
Un utente deve avere un Sistema telefonico con connettività PSTN (una licenza per il piano di chiamata o una licenza Direct Routing OnlineVoiceRoutingPolicy) per essere un delegato o configurare la delega e consentire ad altri utenti di effettuare o ricevere chiamate per loro conto.
...
> [!IMPORTANT]
> Quando l'amministratore tenant disattiva la delega per un utente (dopo che è stato attivato), deve anche pulire le relazioni di delega per quell'utente nell'interfaccia di amministrazione di Teams per evitare un routing delle chiamate non corretto.
...
| Funzionalità | Teams desktop | Teams'app Mac | Teams Web App (Edge) |Teams app per dispositivi mobili iOS/Android | Teams telefono IP |
|------------|---------------|---------------|----------------------|-----------------------------|----------------|
...
La configurazione della delega circolare non è consentita. Se gli utenti con delega hanno anche delega tra di loro, potranno vedere solo la delega e non la delega iniziale.
