...
L'amministratore può scegliere quali altre origini di contenuto didattico (ad esempio SharePoint o origini provider di contenuti di terze parti supportate) saranno disponibili per gli utenti dell'app. L'amministratore configura quindi queste origini per assicurarsi che il contenuto sia disponibile per la ricerca e l'individuazione e possa essere esplorato dai dipendenti che usano l'app.

> [!NOTE]
>  Gli utenti effettuano l'accesso alle risorse di formazione non Microsoft e LinkedIn Learning Pro in un browser o un visualizzatore incorporato. Questa formazione configurata è soggetta alle condizioni separate di licenza, privacy e servizio tra l'organizzazione e le terze parti e non alle condizioni di apprendimento (anteprima). Prima di selezionare questa formazione in Apprendimento (anteprima), verifica di aver raggiunto un accordo per l'organizzazione e gli utenti.
