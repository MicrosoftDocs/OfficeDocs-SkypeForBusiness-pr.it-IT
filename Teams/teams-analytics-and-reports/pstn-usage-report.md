...
Se si hanno piani per chiamate e instradamento diretto, il file esportato potrebbe contenere dati per entrambi i prodotti. Il file di report sull'utilizzo PSTN avrà il nome file "**PSTN.calls. `[UTC date]`.csv**" e Instradamento diretto "**DirectRouting.calls. `[UTC date]`.csv**".
...
> | 4 | Ora di invito | `datetimeoffset` | Quando l'invito iniziale viene inviato Teams uscita da una chiamata dell'utente o del bot a SBC o ricevuto in ingresso a Teams o una chiamata bot dal componente proxy SIP di Routing diretto da SBC |
