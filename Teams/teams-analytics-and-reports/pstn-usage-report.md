---
title: Microsoft Teams report utilizzo PSTN
author: SerdarSoysal
ms.author: serdars
manager: serdars
audience: Admin
...
MS.collection:
- M365-voice
description: Informazioni su come usare il report sull'utilizzo di Teams PSTN nell'interfaccia di amministrazione di Microsoft Teams per ottenere una panoramica dell'utilizzo delle chiamate e delle audioconferenze nell'organizzazione.
appliesto:
- Microsoft Teams
...
1. Nel riquadro di spostamento sinistro dell'interfaccia di amministrazione di Microsoft Teams fare clic su **Analisi &** **reportUsa** >  report. Nella scheda **Visualizza report** , in **Report**, selezionare **Report utilizzo PSTN**.
...
### <a name="direct-routing"></a>Routing diretto
   ...
   ![Screenshot del report di utilizzo PSTN routing diretto nell'interfaccia di amministrazione](../media/teams-reports-pstn-usage-direct-routing-with-callouts.png)![Screenshot del report Utilizzo PSTN routing diretto nell'interfaccia di amministrazione di Microsoft Teams con callout numerati](../media/teams-reports-pstn-usage-direct-routing-with-callouts.png#lightbox)
...
- Chiama da un numero non E.164. Un utente da un PBX di terze parti interconnesso con Direct Routing effettua una chiamata a un utente Teams. In questo caso, il numero del chiamante potrebbe essere qualsiasi numero diverso da E.164, ad esempio +1001. 
...
#### <a name="phone-number-obfuscation"></a>Telefono l'offuscamento dei numeri
...
1.    Utente PSTN 1 in un endpoint PSTN chiamato Teams Utente 1 nel client Teams, tipo di chiamata Dr_In, ID di correlazione 57f28917-42k5-4c0c-9433-79734873f2ac, nessun ID di correlazione condiviso.
2.    Teams Utente 1 su Teams client denominato Utente PSTN 1 in un endpoint PSTN, tipo di chiamata Dr_Out 2c12b8ca-62eb-4c48-b68d-e451f518ff4, nessun ID di correlazione condiviso.
3.    PSTN User 1 in un endpoint PSTN denominato Teams User 2 nel client Teams, tipo di chiamata Dr_In f45e9a25-9f94-46e7-a457-84f5940efde9, ID correlazione condiviso f45e9a25-9f94-46e7-a457-84f5940efde9.
4.    Chiamata 3 esistente con ID di correlazione "f45e9a25-9f94-46e7-a457-84f5940efde9". Utente PSTN 1 in una chiamata con Teams Utente 2. Teams Utente 2 ha trasferito (non vedente o consultante) una chiamata a Teams o utente PSTN, tipo di chiamata Dr_Out_User_Transfer 45a1da7c-9e97-481a-8a05-3fe19a9a77e0, ID correlazione condiviso f45e9a25-9f94-46e7-a457-84f5940efde9.
...
> | 2 | UPN | `nvarchar(128)` | UserPrincipalName (nome di accesso, Azure Active Directory) dell'utente o bot che ha effettuato o ricevuto la chiamata.<br/>In genere corrisponde all'indirizzo SIP dell'utente e può corrispondere all'indirizzo di posta elettronica dell'utente |
...
> | 5 | Ora invito | `datetimeoffset` | Quando l'invito iniziale viene inviato in uscita da Teams chiamata di un utente o bot a SBC o ricevuto in ingresso per Teams o bot chiamata dal componente Proxy SIP di Routing diretto da SBC |
> | 6 | Ora di inizio | `datetimeoffset` | Ora in cui il proxy SIP ha ricevuto la risposta finale (messaggio SIP "200 OK") da SBC in uscita (Teams/Bot a un utente PSTN) o dopo che il proxy SIP ha inviato l'invito all'hop successivo all'interno di Teams back-end alla chiamata in ingresso (Utente PSTN a un Teams/Bot).<br/>Per le chiamate non riuscite e senza risposta, questo può essere uguale al tempo di invito o di errore |
...
> | 11 | Numero chiamante | `nvarchar(32)` | Numero dell'utente o bot che ha effettuato la chiamata. In ingresso a una chiamata dell'utente del team sarà un utente PSTN, in uscita da Teams chiamata dell'utente sarà il numero di utente Teams |
> | 12 | Numero destinatario chiamata | `nvarchar(32)` | Numero dell'utente o del bot che ha ricevuto la chiamata. On inbound to a Team user call it will be the Teams user, onbound from Teams user call it will be the PSTN User |
...
- [Report routing diretto in Microsoft Graph](/graph/api/callrecords-callrecord-getdirectroutingcalls?view=graph-rest-1.0&tabs=http)
