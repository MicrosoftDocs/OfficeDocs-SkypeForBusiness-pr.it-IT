---
title: Microsoft Teams utilizzo PSTN
author: SerdarSoysal
ms.author: serdars
manager: serdars
audience: Admin
...
MS.collection:
- M365-voice
description: Informazioni su come usare il report sull Teams di utilizzo PSTN nell'interfaccia di amministrazione di Microsoft Teams per ottenere una panoramica dell'utilizzo delle chiamate e delle audioconferenze nell'organizzazione.
appliesto:
- Microsoft Teams
...
ms.locfileid: "63556652"
---
# <a name="microsoft-teams-pstn-usage-report"></a>Microsoft Teams utilizzo PSTN
...
1. Nel riquadro di spostamento sinistro dell'interfaccia Microsoft Teams di amministrazione fare clic su **Analisi &** **reportUsage** > . Nella scheda **Visualizza report** , in **Report**, selezionare **Report utilizzo PSTN**.
...
- Chiama da un numero diverso da E.164. Un utente di un PBX di terze parti connesso con Il routing diretto effettua una chiamata a un Teams utente. In questo caso, il numero chiamante potrebbe essere qualsiasi numero diverso da E.164, ad esempio +1001. 
...
2.    Teams Utente 1 nel client Teams denominato Utente PSTN 1 in un endpoint PSTN, tipo di chiamata Dr_Out 2c12b8ca-62eb-4c48-b68d-e451f518ff4, nessun ID di correlazione condiviso.
...
Se si hanno piani per chiamate e instradamento diretto, il file esportato potrebbe contenere dati per entrambi i prodotti. Il file di report sull'utilizzo PSTN avrà il nome del file "**PSTN.calls.`[UTC date]`.csv**" e Instradamento diretto "**DirectRouting.calls.`[UTC date]`.csv**".
...
> | # | Nome | [Tipo di dati (SQL Server)](/sql/t-sql/data-types/data-types-transact-sql) | Descrizione |
> | :-: | :-: | :-: |:------------------- |
> | 0 | CorrelationId | `uniqueidentifier` | Identificatore di chiamata. Più punti della stessa chiamata possono condividere lo stesso CorrelationId |
> | 1 | AAD ObjectId | `uniqueidentifier` | Id utente chiamante in Azure Active Directory.<br/> Questa e altre informazioni utente possono essere Null/vuote per i tipi di chiamata bot |
> | 2 | UPN | `nvarchar(128)` | UserPrincipalName (nome di accesso, Azure Active Directory) dell'utente o del bot che ha effettuato o ricevuto la chiamata.<br/>In genere corrisponde all'indirizzo SIP dell'utente e può essere uguale all'indirizzo di posta elettronica dell'utente |
> | 3 | Nome visualizzato | `nvarchar(128)` | Il nome di un utente o di un bot chiamante (ad esempio coda di chiamata o Operatore automatico) come impostato in interfaccia di amministrazione di Microsoft 365 |
> | 4 | Paese utente | `nvarchar(2)` | Codice paese dell'utente, [ISO 3166-1 alfa-2](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2) |
> | 5 | Ora di invito | `datetimeoffset` | Quando l'invito iniziale viene inviato Teams uscita da una chiamata utente o bot a SBC o ricevuto in ingresso a Teams o una chiamata bot dal componente proxy SIP di Routing diretto da SBC |
> | 6 | Ora di inizio | `datetimeoffset` | Ora in cui il proxy SIP ha ricevuto la risposta finale (messaggio SIP "200 OK") da SBC in uscita (Teams/Bot a un utente PSTN) o dopo che il proxy SIP ha inviato l'invito all'hop successivo all'interno di Teams backend alla chiamata in ingresso (Utente PSTN a un Teams/Bot).<br/>Per le chiamate non riuscite e senza risposta, questo può essere uguale al tempo di invito o di errore |
> | 7 | Tempo di errore | `datetimeoffset` | Esiste solo per le chiamate non riuscite (non completamente stabilite) |
> | 8 | Ora di fine | `datetimeoffset` | Esiste solo per le chiamate riuscite (completamente stabilite). Ora di fine della chiamata |
> | 9 | Durata (secondi) | `int` | Durata della chiamata, dall'invito alla fine o all'errore della chiamata. Per l'inoltro di chiamata, la durata include lo squillo nella coda di chiamata. |
> | 10 | Operazione riuscita | `nvarchar(3)` | Sì/No. Operazione riuscita o tentativo |
> | 11 | Numero chiamante | `nvarchar(32)` | Numero dell'utente o del bot che ha effettuato la chiamata. In ingresso a una chiamata utente del team sarà un utente PSTN, in uscita da Teams chiamata utente sarà il numero Teams utente |
> | 12 | Numero del chiamato | `nvarchar(32)` | Numero dell'utente o del bot che ha ricevuto la chiamata. In ingresso a una chiamata utente del team sarà l'utente Teams, in uscita da Teams utente sarà l'utente PSTN |
...
> | 15 | Area geografica di Azure per la segnalazione | `nvarchar(8)` | Data center usato per la segnalazione sia per le chiamate di bypass che per le chiamate non bypass |
...
- [Report delle chiamate PSTN in Microsoft Graph](/graph/api/callrecords-callrecord-getpstncalls?view=graph-rest-1.0&tabs=http)
