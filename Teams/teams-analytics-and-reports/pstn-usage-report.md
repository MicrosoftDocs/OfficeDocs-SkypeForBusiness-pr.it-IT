...
> | 4 | Ora di invito | `datetimeoffset` | Quando l'invito iniziale viene inviato Teams uscita da una chiamata utente o bot a SBC o ricevuto in ingresso a Teams o una chiamata bot dal componente proxy SIP di Routing diretto da SBC |
> | 5 | Ora di inizio | `datetimeoffset` | Ora in cui il proxy SIP ha ricevuto la risposta finale (messaggio SIP "200 OK") dal servizio SBC in uscita (Teams/Bot a un utente PSTN) o dopo che il proxy SIP ha inviato l'invito all'hop successivo all'interno di un backend di Teams su una chiamata in ingresso (Utente PSTN a un Teams/Bot).<br/>Per le chiamate non riuscite e senza risposta, questo può essere uguale al tempo di invito o di errore |
