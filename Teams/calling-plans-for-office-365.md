...
Le chiamate ad altri utenti di Skype for Business e Microsoft Teams sono gratuite, ma se si vuole che gli utenti possano chiamare i normali telefoni e non si ha già un provider di servizi per effettuare chiamate vocali, è necessario acquistare Piani per chiamate di Microsoft Teams. Per ulteriori informazioni, vedi [Sistema telefonico e Piani per chiamate](calling-plan-landing-page.md).
...
- **Piano per chiamate con pagamento** automatico: gli utenti con licenza possono effettuare chiamate a numeri nel paese o nell'area geografica in cui all'utente è assegnata la licenza microsoft 365 in base alla posizione dell'utente e i numeri internazionali in  [196 paesi/aree geografiche](country-and-region-availability-for-audio-conferencing-and-calling-plans/users-can-make-outbound-calls-to-these-countries-and-regions.md). Sono inclusi minuti illimitati per le chiamate in arrivo. Non sono inclusi minuti in uscita. Tutte le chiamate in uscita vengono addebitate in base ai minuti utilizzati, sia con i Crediti comunicazioni che con la fatturazione post-utilizzo (solo per le nuove sottoscrizioni di chiamate per le nuove esperienze di commercio). Le licenze vengono assegnate agli utenti in base alla loro posizione.

  Per informazioni su come finanziare e utilizzare i minuti per un piano per chiamate con pagamento in base al consumo, vedi [Come finanziare un piano per chiamate con pagamento in base al consumo](#how-to-fund-a-pay-as-you-go-calling-plan).

  Sono disponibili due opzioni di **piano per chiamate** con pagamento in base al metodo di pagamento:

  - **Zona 1 piano per chiamate con** pagamento in base al costo: acquistalo per gli utenti di Stati Uniti e Portorico, Canada e Regno Unito.
...
> [!NOTE]
> Il pooling è disponibile solo per piani per chiamate identici.
...
## <a name="how-to-buy-calling-plans"></a>Come acquistare piani per chiamate
...
## <a name="how-to-fund-a-pay-as-you-go-calling-plan"></a>Come finanziare un piano per chiamate con pagamento in base al costo

I piani per chiamate a pagamento non hanno un numero impostato di minuti di chiamate in uscita, ad esempio i piani per chiamate nazionali o internazionali. Alcuni amministratori potrebbero invece dover eseguire ulteriori passaggi per finanziare il proprio piano per le chiamate con pagamento in base al metodo di pagamento. Fino a quando il piano per chiamate pay-as-you-go non viene finanziato correttamente, gli utenti non saranno in grado di effettuare chiamate in uscita.

Esistono due modi per finanziare e utilizzare i minuti con un piano per chiamate con pagamento in base al consumo:

- **Acquistare crediti comunicazioni**
  - Questa opzione è per i clienti che non hanno acquistato il piano per chiamate tramite la nuova esperienza di commercio (NCE). Questi clienti devono acquistare Crediti comunicazioni per finanziare il proprio piano per chiamate con pagamento in base al consumo.
  - Per istruzioni su come acquistare e assegnare Crediti comunicazioni, vedi [Configurare i Crediti comunicazioni per l'organizzazione](set-up-communications-credits-for-your-organization.md).

- **Acquista il piano per chiamate con pagamento in base al consumo tramite la nuova esperienza di commercio (NCE)**
  - I clienti che hanno acquistato il piano per le chiamate tramite la nuova esperienza di commercio possono pagare i minuti delle chiamate in uscita che hanno usato nel mese precedente. Questa opzione è detta anche fatturazione post-utilizzo o post-consumo.
  - Per istruzioni sull'acquisto di un piano per chiamate tramite la nuova esperienza di commercio, vedi [Abilitare il pagamento in base al consumo per l'abbonamento](/microsoft-365/commerce/subscriptions/manage-pay-as-you-go-services) e [Eccedenza del nuovo commercio per telco pay-as-you-go](/partner-center/new-commerce-telco-payg).
...
- [Prezzi per le audioconferenze](https://www.microsoft.com/microsoft-365/microsoft-teams/online-meetings).
...
- [Configurare Cloud Voicemail - guida Amministrazione](set-up-phone-system-voicemail.md).
- [Configurare i piani per chiamate](set-up-calling-plans.md).
- [Aggiungi fondi e gestisci i Crediti comunicazioni](add-funds-and-manage-communications-credits.md).
