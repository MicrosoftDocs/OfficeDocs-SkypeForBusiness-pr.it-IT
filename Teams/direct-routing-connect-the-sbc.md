---
title: Connessione il session border controller (SBC) al routing diretto
...
f1.keywords:
- NOCSH
description: Informazioni su come configurare e connettere il servizio SBC a Teams Sistema telefonico routing diretto.
...
---
# <a name="connect-your-session-border-controller-sbc-to-direct-routing"></a>Connessione il session border controller (SBC) al routing diretto

Questo articolo descrive come configurare un session border controller (SBC) e connetterlo a Direct Routing.  Questo è il passaggio 1 della procedura seguente per configurare il routing diretto:

- **Passaggio 1. Connessione SBC con Sistema telefonico e convalidare la connessione** (questo articolo)
...
3. Immettere un NOME DI DOMINIO completo per SBC. <br><br>Verificare che la parte relativa al nome di dominio dell'FQDN corrisponda a un dominio registrato nel tenant `*.onmicrosoft.com` e tenere presente che il nome di dominio non è supportato per il nome di dominio FQDN SBC. Ad esempio, se si hanno due nomi di dominio e `contoso.com` `contoso.onmicrosoft.com`, usare `sbc.contoso.com` come nome SBC. Se si usa un sottodominio, assicurarsi che questo sottodominio sia registrato anche nel tenant. Ad esempio, se si vuole usare `sbc.service.contoso.com`, è `service.contoso.com` necessario registrare.
...
### <a name="connect-the-sbc-to-the-tenant"></a>Connessione il database SBC al tenant

Per connettere SBC al tenant, usare il cmdlet [New-CsOnlinePSTNGateway](/powershell/module/skype/new-csonlinepstngateway) . In una sessione di PowerShell digitare quanto segue e quindi premere INVIO:
...
|Obbligatorio?|Teams'interfaccia di amministrazione|Parametro di PowerShell|Descrizione|Impostazione predefinita|Valori possibili|Tipo e restrizioni|
|:-----|:-----|:-----|:-----|:-----|:-----|:-----|
...
|No|**Cronologia chiamate inoltrate**|ForwardCallHistory |Indica se le informazioni della cronologia chiamate vengono inoltrate tramite il trunk. Quando si attiva questa opzione, il proxy Microsoft 365 invia un'intestazione History-info e Referred-by. |Falso|Vero<br/>Falso|Boolean|
