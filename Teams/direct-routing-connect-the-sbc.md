...
---
# <a name="connect-your-session-border-controller-sbc-to-direct-routing"></a>Connettere il controller dei confini della sessione (SBC) all'instradamento diretto

Questo articolo descrive come configurare un session border controller (SBC) e collegarlo all'instradamento diretto del sistema telefonico.  Questo è il passaggio 1 della procedura seguente per configurare il routing diretto:

- **Passaggio 1. Collegare il proprio SBC al Sistema telefonico e convalidare la connessione** (questo articolo)
...
3. Immettere un nome di dominio completo per SBC. <br><br>Verificare che la parte relativa al nome di dominio completo corrisponda a un dominio registrato nel tenant e tenere presente che il nome di dominio non è supportato per il nome di dominio `*.onmicrosoft.com` COMPLETO SBC. Ad esempio, se si hanno due nomi di `contoso.com` dominio e , usare come nome `contoso.onmicrosoft.com` `sbc.contoso.com` SBC. Se si usa un sottodominio, verificare che sia registrato anche nel tenant. Ad esempio, se si vuole usare `sbc.service.contoso.com` , è `service.contoso.com` necessario registrare.
...
2. [Connettere l'SBC al tenant.](#connect-the-sbc-to-the-tenant)
...
Dopo aver stabilito una sessione remota di PowerShell, verificare che sia possibile visualizzare i comandi per gestire il servizio SBC. Per verificare i comandi, digitare oppure copiare e incollare il comando seguente nella sessione di PowerShell e quindi premere INVIO: 
...
```
  ...
  > [!NOTE]
  > 1. È consigliabile impostare un limite massimo per le chiamate nel servizio SBC usando le informazioni disponibili nella documentazione di SBC. Il limite attiverà una notifica se il valore SBC è al livello di capacità.
  > 2. È possibile connettere il servizio SBC solo se la parte del dominio del relativo FQDN corrisponde a uno dei domini registrati nel tenant, ad eccezione di \* onmicrosoft.com. L'uso onmicrosoft.com di dominio completo SBC non è \* supportato. Ad esempio, se si hanno due nomi di dominio, **contoso.com** e **contoso.onmicrosoft.com,** è possibile usare sbc.contoso.com per il nome SBC. Se si prova a connettere il dominio SBC con un nome come sbc.contoso.abc, il sistema non lo consente perché il dominio non è di proprietà del tenant.<br/>
...
|No|**Intestazione PAI (Forward P-Asserted-identity)**|ForwardPAI|Indica se l'intestazione PAI viene inoltrata insieme alla chiamata. L'intestazione PAI consente di verificare l'identità del chiamante. Se questa impostazione è attivata, viene inviata anche l'intestazione Privacy:ID.|Falso|Vero<br/>Falso|Boolean|
