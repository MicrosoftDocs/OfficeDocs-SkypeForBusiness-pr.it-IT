...
Prima di creare lo schema che definisce la gerarchia, è necessario eseguire alcune pianificazioni e decidere come modellare l'organizzazione.  Una delle prime priorità è decidere quali gruppi dell'organizzazione devono pubblicare le attività in altri gruppi. Ogni nodo nella gerarchia rappresenta un gruppo di lavoro o un gruppo di gruppi.
...
* Se un utente non è membro di un team nella gerarchia, non ne visualizza le funzionalità correlate alla pubblicazione.

### <a name="guidelines"></a>Linee guida

* Per ogni organizzazione può essere applicato un solo file della gerarchia. Tuttavia, è possibile includere diverse parti dell'organizzazione come gerarchie distinte di nodi all'interno di un file CSV. Ad esempio, Contoso Pharmaceuticals ha un nodo radice Farmacia e un nodo radice Di vendita al dettaglio. Entrambi i nodi radice hanno più righe di elementi discendenti e non esiste alcuna sovrapposizione tra di essi.
...
Lo schema che definisce la gerarchia si basa su un file con valori delimitati da virgole (CSV). Ogni riga nel file CSV corrisponde a un nodo all'interno della gerarchia dei team. Ogni riga contiene informazioni che nominano il nodo all'interno della gerarchia, lo collegano facoltativamente a un team e includono attributi che possono essere usati per filtrare i team nelle app che lo supportano.
...
### <a name="add-required-columns"></a>Aggiungere le colonne necessarie
...
* È consigliabile non includere informazioni riservate nei nomi dei contenitori. Al momento, i team di pubblicazione non possono rimuovere un contenitore tramite pubblicazione dopo la creazione.
...
* Tre colonne di attributi `Store layout` denominate `Departments:Clothing` , , e `Departments:Foods`
* Tre colonne del contenitore `Fresh Foods` `Frozen Foods` denominate, , e `Women's Wear`

`Store layout`L'attributo ha valori che includono , e `Compact` `Standard` `Large` . Le `Departments` colonne degli attributi possono essere impostate su un valore pari a `0` (zero) o `1` . Il `Store` layout e gli attributi non sono visualizzati `Departments` nell'immagine precedente. Vengono aggiunti qui per mostrare in che modo è possibile aggiungere attributi alle voci del nodo. Lo stesso vale per le colonne di tre contenitori.
