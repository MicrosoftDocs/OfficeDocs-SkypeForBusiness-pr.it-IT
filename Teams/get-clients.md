...
Gli amministratori IT possono scegliere il metodo preferito per distribuire i file di installazione nei computer dell'organizzazione. Alcuni esempi includono Microsoft endpoint Configuration Manager (Windows) o grafp Pro (macOS). Per ottenere il pacchetto MSI per la distribuzione di Windows, vedere [installare Microsoft teams con MSI](msi-deployment.md).  
...
> [!NOTE] 
> Se si trova un bug, inviarlo tramite `Report a Problem` dall'interno del client. Per i problemi noti, vedere [problemi noti](Known-issues.md).
> Per il supporto di teams for Linux puoi usare il [canale di supporto di Linux Forum su Microsoft Q&a](https://docs.microsoft.com/answers/topics/teams.html). Assicurati di usare il `teams-linux` tag per pubblicare domande. 
...
2. Installare utilizzando una delle opzioni seguenti:  
    - Aprire lo strumento di gestione dei pacchetti pertinente e passare al processo di installazione dell'app Linux auto-guidata.
...
#### <a name="install-manually-from-the-command-line"></a>Installare manualmente dalla riga di comando

Installare manualmente sulle distribuzioni Debian e Ubuntu:
```
...
-   **Android**: il supporto è limitato alle ultime quattro versioni principali di Android. Quando viene rilasciata una nuova versione principale di Android, la nuova versione e le tre versioni precedenti sono supportate ufficialmente.

-   **iOS**: il supporto è limitato alle due versioni principali più recenti di iOS. Quando viene rilasciata una nuova versione principale di iOS, la nuova versione di iOS e la versione precedente sono supportate ufficialmente.
