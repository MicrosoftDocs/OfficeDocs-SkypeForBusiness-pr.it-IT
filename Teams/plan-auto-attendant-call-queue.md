...
- Una licenza per i servizi di audioconferenza o un criterio di instradamento vocale online assegnato se la coda di chiamata usa la modalità conferenza

Se i tuoi agenti usano l'app Microsoft Teams per le chiamate in coda di chiamata, devono essere in modalità TeamsOnly.

Quando si trasferiscono chiamate a un numero di telefono esterno, l'account della risorsa che esegue il trasferimento (ovvero quello associato all'operatore automatico o alla coda di chiamata) deve avere un numero di telefono e una licenza Utente virtuale per Sistema telefonico Microsoft 365. Inoltre:

- Per un account di risorsa con un numero di Piano per chiamate, assegnare una [licenza di Piano per chiamate.](calling-plans-for-office-365.md)
- Per un account di risorsa con un numero di instradamento diretto, assegnare un [criterio di routing vocale online.](manage-voice-routing-policies.md)
...
> I trasferimenti tra i trunk del Piano per chiamate e i trunk di routing diretto non sono supportati.
...
- Vuoi consentire agli agenti in coda di rifiutare esplicitamente di effettuare le chiamate?
...
- **Chiamare le** persone per nome o numero interno: i chiamanti possono essere indirizzati a comporre il numero di interno della persona che stanno cercando di raggiungere nell'elenco dell'organizzazione o digitando il nome della persona.
- **Disconnetti:** un operatore automatico può interrompere la chiamata.

> [!NOTE]
> Un unico operatore automatico può supportare un solo metodo di chiamata per.  Per consentire ai chiamanti di comporre per nome e per numero, sarà necessario creare un operatore automatico che abbia un'opzione per comporre per nome e l'altro per chiamata per interno.  Ognuna di queste opzioni instraderà all'operatore automatico distinto configurato per questi scenari di chiamata da.
...
- Il tasto due (2) reindirizza i chiamanti alla coda di chiamata di supporto. Questa coda di chiamata è collegata a un team che contiene il team di supporto assegnato al team.
...
- L'appartenenza per ogni coda di chiamata. È possibile aggiungere utenti singolarmente o mappare la coda a diversi tipi di gruppi. Il mapping di una coda a un team offre l'esperienza più versatile.
...
- Mantenere l'elenco delle voci di menu su un valore non inferiore a cinque. I chiamanti possono avere difficoltà a ricordare più di cinque opzioni. Usare operatori automatici annidati se sono necessarie più opzioni per instradare correttamente una chiamata.
...
- Evitare frequenti aggiornamenti al routing delle chiamate. Se in futuro cambi le opzioni di menu per un operatore automatico, chiamali nei comandi vocali per i primi 30 giorni.
...
1. Ottieni i numeri di servizio necessari per gli operatori automatici e le code di chiamata che desideri siano accessibili componendo direttamente dall'esterno della tua organizzazione. Può includere il [trasferimento di numeri da un altro provider o](phone-number-calling-plans/transfer-phone-numbers-to-teams.md) la richiesta di nuovi numeri di [servizio.](getting-service-phone-numbers.md)
...
3. [Crea un account delle risorse](manage-resource-accounts.md) per ogni operatore automatico e coda di chiamata che desideri creare. Assegnare a ogni account una licenza Sistema telefonico - Utente virtuale e, facoltativamente, un numero di servizio.

4. [Creare le festività](set-up-holidays-in-teams.md) per le quali si vuole impostare l'instradamento delle chiamate separato negli operatori automatici.

5. Facoltativamente, [impostare il parco chiamate e il](call-park-and-retrieve.md) recupero se si vuole usare questa funzionalità per facilitare i trasferimenti di chiamata.
