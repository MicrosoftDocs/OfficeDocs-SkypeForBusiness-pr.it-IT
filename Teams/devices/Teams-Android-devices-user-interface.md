...
Se si [](/microsoftteams/user-access) assegna una licenza utente a un account, ad esempio una licenza E3 o E5, il dispositivo Teams visualizza l'interfaccia utente finale predefinita, completamente in primo piano per la maggior parte degli scenari utente. Tuttavia, se un dispositivo esegue una funzione specifica, ad esempio un telefono di area comune o una sala riunioni, esistono interfacce utente specifiche per questi utilizzi.
...
All'account utente viene assegnata una licenza E5. Si tratta di una licenza utente, quindi il dispositivo mostra l'interfaccia utente finale predefinita:
...
In questa immagine all'account utente è stata assegnata una [licenza area Telefono comune](/microsoftteams/set-up-common-area-phones). I telefoni dell'area comune vengono usati principalmente per effettuare e ricevere chiamate telefoniche. Di conseguenza, la tastiera del telefono viene visualizzata sullo schermo:
...
Di seguito è riportato un esempio di come ignorare il rilevamento automatico delle licenze. In questo esempio si supponga che a un account delle risorse della sala riunioni denominato conf-adams@contoso.com sia stata assegnata una licenza E3. Quando l'account è connesso, si vuole che gli utenti vedano l'interfaccia utente della sala riunioni.
...
Microsoft Teams'interfaccia di amministrazione consente di gestire Microsoft Teams dispositivi. Per altre informazioni sulla gestione dei dispositivi Teams'interfaccia di amministrazione, vedere [Gestire i dispositivi in Microsoft Teams](device-management.md).
...
Se si vuole che un telefono venga visualizzato in una sezione diversa, è possibile assegnare una licenza diversa al telefono oppure creare e assegnare un criterio di Teams ip Telefono come descritto in precedenza[.](#override-automatic-user-interface-detection)
