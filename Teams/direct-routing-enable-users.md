...
Il routing diretto richiede che l'utente sia ospitato online. È possibile verificare esaminando il parametro RegistrarPool, che deve avere un valore nel infra.lync.com dominio. Microsoft consiglia, ma non richiede, di modificare lineuri da locale a online quando si esegue la migrazione degli utenti a Teams Direct Routing. 
...
2. Eseguire il comando: 
    ...
    ``` 
    Se OnPremLineUri viene popolato con un numero di telefono <E.164>, il numero di telefono è stato assegnato in locale e sincronizzato con Microsoft 365. Se si vuole gestire il numero di telefono online, pulire il parametro usando Skype for Business Management Shell locale e eseguire la sincronizzazione con Microsoft 365 prima di configurare il numero di telefono usando Teams PowerShell. 
...
Dopo aver creato l'utente e aver assegnato una licenza, è necessario configurare le impostazioni del telefono online dell'utente. Si noti che la configurazione Cloud Voicemail per l'utente è automatica. Non è necessario eseguire altre operazioni di configurazione.

È possibile configurare il numero di telefono usando l'interfaccia Teams di amministrazione o usando Teams PowerShell.

### <a name="use-teams-admin-center"></a>Usare Teams di amministrazione

1. Passare a **UtentiManage** ->  utenti.

2. Selezionare un utente.

2. In **Informazioni** **generali account** selezionare **Modifica**.

3. In **Assegna numero di telefono** selezionare Instradamento **diretto dal** menu Telefono tipo di **numero.**

4. Immettere un numero di telefono assegnato e un'estensione del numero di telefono, se possibile.
...
1. Connessione a una sessione Microsoft Teams PowerShell. 

2. I passaggi successivi variano a seconda che si gestirà il numero di telefono dell'utente in locale o online. Se si gestisce il numero di telefono locale, è necessario usare la shell di gestione locale di Skype for Business Management Shell, il Pannello di controllo o uno dei metodi descritti in Decidere come gestire gli attributi dopo la [decommisionazione](/skypeforbusiness/hybrid/cloud-consolidation-managing-attributes).
