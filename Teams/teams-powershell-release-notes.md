...
|------- | -------------------- | ------------------------------ |
| Agosto 2021 | [2.5.0](https://www.powershellgallery.com/packages/MicrosoftTeams/2.5.0) |<li>Il cmdlet New-Team è stato aggiornato con un'API più recente per evitare gli errori di creazione del team osservati in precedenza. Di conseguenza, il numero di errori e la latenza del cmdlet è stato migliorato.</li><li>L'accesso al token di accesso Connect-MicrosoftTeams ora usa una matrice di token unificati invece di parametri separati per ogni token specifico della risorsa. Altre informazioni sono disponibili [qui.](/powershell/module/teams/connect-microsoftteams?view=teams-ps)</li><li>L'errore di accesso interattivo Connect-MicrosoftTeams in Cloudshell è stato risolto. L'impostazione predefinita è ora l'uso dell'identità di accesso dell'utente invece di richiedere di eseguire di nuovo l'autenticazione.</li><li>I cmdlet TeamsUnassignedNumberTreatment sono ora disponibili.</li><li>Get-CsOnlineDialInConferencingBridge e Set-CsOnlineDialInConferencingBridge cmdlet sono stati migrati dall'implementazione precedente alle API più recenti.</li><li>Sono state rilasciate versioni modernizzate Get-CsTenant e Get-CsOnlineUser (solo con il parametro -identity). Non generano più proprietà deprecate e hanno alcune modifiche di formattazione rispetto alle loro controparti remote.</li>|
