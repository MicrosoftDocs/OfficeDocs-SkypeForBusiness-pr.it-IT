...
Microsoft School Data Sync (detto anche SDS) fornisce dati al Sistema informativo dell’istituto di istruzione (detto anche SIS), la struttura gerarchica del sistema educativo e i mapping a cui l'utente è assegnato, oltre a fornire dati aggiuntivi sulla gerarchia degli studenti e dell'organizzazione.
...
- Assicurarsi che **la reale e completa gerarchia dell’organizzazione corrisponda**. In alcuni casi, questa gerarchia non si riflette nel sistema informativo degli studenti (SIS). In tal caso deve essere aggiunta manualmente al file CSV prima della sincronizzazione.
